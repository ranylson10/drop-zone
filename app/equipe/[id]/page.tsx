...
type MembroEquipeNormalizado = Omit<
  MembroEquipe,
  "perfis_jogo" | "profiles"
> & {
...
function normalizarProfile(
  profile: ProfileResumo | ProfileResumo[] | null | undefined,
): ProfileResumo | null {
  if (!profile) return null;
  return Array.isArray(profile) ? profile[0] || null : profile;
}

function normalizarPerfilJogo(
  perfil: PerfilJogo | PerfilJogo[] | null,
): (PerfilJogo & { profile: ProfileResumo | null }) | null {
...
async function carregarEstatisticasEquipe(
  ...
  const campeonatoIds = Array.from(
    new Set(
      inscricoes
        .map((item: any) => String(item.campeonato_id || ""))
        .filter(Boolean),
    ),
  );

  const inscricaoIdsEquipe = new Set(
    inscricoes.map((item: any) => String(item.id || "")).filter(Boolean),
  );
  ...
  const [participantesRes, resultadosRes] = await Promise.all([
    supabase
      .from("campeonato_equipes")
      .select(
        "id, campeonato_id, equipe_id, equipe_avulsa_id, line_id, nome_exibicao",
      )
  ...
  if (participantesRes.error) {
    console.warn(
      "Erro ao carregar participantes dos campeonatos:",
  ...
  inscricoes.forEach((item: any) => {
    const camp = Array.isArray(item.campeonatos)
      ? item.campeonatos[0]
      : item.campeonatos;
    campeonatoNomeMap.set(
      String(item.campeonato_id),
      camp?.nome || "Campeonato sem nome",
    );
  });

  const participanteIds = new Set(
    participantes.map((item) => String(item.id || "")).filter(Boolean),
  );
  const publicToCampeonatoEquipeId = new Map<string, string>();

  participantes.forEach((item) => {
    const ceId = String(item.id || "").trim();
    [item.equipe_id, item.equipe_avulsa_id, item.line_id].forEach(
      (publicId) => {
  ...
  campeonatoIds.forEach((campeonatoId) => {
    ...
    const melhorDaEquipe = ranking.find((item) =>
      inscricaoIdsEquipe.has(item.campeonato_equipe_id),
    );
    ...
    const posicao =
      ranking.findIndex(
        (item) =>
          item.campeonato_equipe_id === melhorDaEquipe.campeonato_equipe_id,
      ) + 1;
  ...
  const podios = historico.filter(
    (item) => Number(item.posicao || 0) > 0 && Number(item.posicao || 0) <= 3,
  ).length;
  ...
  return {
    ...
    mediaPosicao:
      posicoesValidasEquipe > 0
        ? somaPosicoesEquipe / posicoesValidasEquipe
        : 0,
...
export default function PerfilEquipePage() {
  ...
  const [estatisticas, setEstatisticas] =
    useState<EstatisticasEquipe>(estatisticasVazias);
  const [historicoAberto, setHistoricoAberto] = useState<
    "titulos" | "historico" | null
  >(null);
 ...
 `,
      ...
      console.error(
        "Erro ao carregar perfil da equipe:",
        error?.message || error,
      );
    } finally {
      setLoading(false);
  ...
  const meuVinculo = useMemo(() => {
    ...
    return (
      membrosNormalizados.find(
        (m) => m.user_id === user.id || m.perfil?.user_id === user.id,
      ) || null
    );
  ...
  const podeGerenciar = useMemo(() => {
    ...
    if (equipe.criado_por === user.id) return true;
    return (
      meuVinculo?.tipo === "dono" ||
      meuVinculo?.tipo === "admin" ||
      meuVinculo?.tipo === "manager"
    );
  }, [equipe, meuVinculo, user?.id]);
  ...
  const lideres = useMemo<LiderComando[]>(
    () =>
      membrosNormalizados
        .filter(
          (m) =>
            m.tipo === "dono" || m.tipo === "admin" || m.tipo === "manager",
        )
        .map((m) => ({
          id: m.id,
          tipo: m.tipo,
  ...
  const bandeira = useMemo(
    () => getCountryFlagEmoji(equipe?.pais),
    [equipe?.pais],
  );
  ...
  const listaHistoricoAberta =
    historicoAberto === "titulos"
      ? estatisticas.titulosLista
      ...
      <div className="mx-auto max-w-[1500px] border-x border-zinc-200 bg-white max-md:border-0">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 max-md:block">
                  <div className="flex flex-wrap items-center gap-4 text-[12px] text-[#142340] max-md:grid max-md:grid-cols-4 max-md:gap-0 max-md:text-[11px]">
                    ...
                    <span className="inline-flex items-center gap-1">
                      ...
                      <strong className="font-semibold">
                        {formatarData(equipe.data_fundacao)}
                      </strong>
                    </span>
                    ...
                    <span
                      ...
                      title={equipe.pais || "País não informado"}
                    >
                      {bandeira}
                    </span>
                  </div>
          ...
          <div className="border border-zinc-200 bg-white p-4 max-md:border-0 max-md:p-0">
            <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between max-md:mb-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  onClick={() =>
                    setHistoricoAberto(
                      historicoAberto === "titulos" ? null : "titulos",
                    )
                  }
                ...
                <button
                  ...
                  onClick={() =>
                    setHistoricoAberto(
                      historicoAberto === "historico" ? null : "historico",
                    )
                  }
                  className="border border-zinc-300 bg-white px-3 py-2 text-[12px] font-medium text-[#142340] hover:bg-zinc-50"
                >
            ...
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-8 max-md:grid-cols-4 max-md:gap-1">
              {[
                ...
                {
                  label: "Média Kills",
                  value: estatisticas.mediaAbates.toFixed(1),
                },
                {
                  label: "Taxa título",
                  value: `${estatisticas.taxaTitulos.toFixed(0)}%`,
                },
              ].map((item) => (
                <div
                  key={item.label}
                  className="border border-zinc-200 bg-[#f8fafc] p-3 max-md:border-0 max-md:bg-zinc-50 max-md:p-2"
                >
                    ...
                    <table className="w-full border-collapse text-[13px]">
                      ...
                      <tbody>
                        {listaHistoricoAberta.map((item) => (
                          <tr
                            key={`${historicoAberto}-${item.campeonato_id}`}
              ...
              return (
                <button
                  ...
                  className={`relative inline-flex items-center gap-2 pb-4 text-[16px] font-semibold uppercase tracking-[0.2em] transition ${
                    ativa
                      ? "text-[#2563eb]"
                      : "text-[#8ea0be] hover:text-[#142340]"
                  }`}
                >
                  <Icon size={16} />
                  {tab.label}
                  {ativa ? (
                    <span className="absolute inset-x-0 bottom-0 h-[3px] bg-[#2563eb]" />
                  ) : null}
                </button>
              );
            })}
        ...
        <section className="border-t border-zinc-200 px-6 py-8 md:px-10">
          ...
          <p className="mt-6 max-w-4xl text-[16px] font-semibold leading-8 text-[#142340]">
            {equipe.descricao?.trim() ||
              "Sem descrição cadastrada para esta equipe."}
          </p>
        </section>
      </div>
    </div>
