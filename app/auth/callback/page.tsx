    ...
    async function finalizarLogin() {
      try {
        ...
        if (data.user) {
          ...
          const username = meta.username || meta.name || meta.full_name || data.user.email?.split('@')[0]
          await ensureUserProfile({ userId: data.user.id, email: data.user.email, username })
        }
