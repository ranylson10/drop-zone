  ...
  return (
    <div className="border border-amber-200 bg-amber-50 p-5 text-sm font-semibold text-amber-800">
      <div className="text-[11px] font-black uppercase tracking-[0.18em] text-amber-700">
        Edição de jogadores bloqueada
      </div>
      <p className="mt-2 leading-6">
        A aba fica disponível, mas adicionar, trocar ou remover jogadores é
        permitido apenas para o dono do campeonato ou ajudantes autorizados.
      </p>
    </div>
  );
}
...
export default function AbaJogadores({
  ...
  const [filtroOrigem, setFiltroOrigem] = useState<"todas" | OrigemJogador>(
    "todas",
  );

  const [equipesInscritas, setEquipesInscritas] = useState<
    EquipeComJogadores[]
  >([]);
  const [listaGeralJogadores, setListaGeralJogadores] = useState<
    JogadorFormatado[]
  >([]);
  ...
  const [metodoRecrutamento, setMetodoRecrutamento] = useState<
    "pesquisar" | "criar_avulso" | null
  >(null);
  const [equipeAlvoInscricaoId, setEquipeAlvoInscricaoId] = useState<
    string | undefined
  >(undefined);
  const [equipeAlvoId, setEquipeAlvoId] = useState<string | undefined>(
    undefined,
  );
  const [equipeAlvoAvulsaId, setEquipeAlvoAvulsaId] = useState<
    string | undefined
  >(undefined);
  ...
  const [jogadorParaTroca, setJogadorParaTroca] =
    useState<JogadorFormatado | null>(null);
  ...
  const [perfilTrocaSelecionado, setPerfilTrocaSelecionado] =
    useState<PerfilJogo | null>(null);
  ...
  const carregarDados = useCallback(async () => {
    ...
    try {
      ...
      const idsEquipesOficiais = inscricoesRows
        .map((i: any) => i.equipe_id)
        .filter(Boolean);
      const idsEquipesAvulsas = inscricoesRows
        .map((i: any) => i.equipe_avulsa_id)
        .filter(Boolean);
      ...
      const [
        ...
        idsEquipesOficiais.length > 0
          ? supabase
              .from("equipes")
              .select("id, nome, tag, logo_url, codigo_publico")
        ...
        lineIds.length > 0
          ? supabase
              ...
              .select(
                "id,line_id,perfil_jogo_id,jogador_avulso_id,tipo_slot,ordem,funcao_line,created_at",
              )
              .in("line_id", lineIds)
      ...
      const [
        ...
        perfilIds.length > 0
          ? supabase
              .from("perfis_jogo")
              .select(
                "id, nick, uid_jogo, foto_capa, funcao, equipe_id, user_id, ativo",
              )
              .in("id", perfilIds)
          : Promise.resolve({ data: [], error: null }),
        jogadorAvulsoIds.length > 0
          ? supabase
              ...
              .select(
                "id, campeonato_id, equipe_id, equipe_avulsa_id, nick, uid_jogo, funcao, foto_url, criado_por",
              )
      ...
      const avulsosMap = new Map<string, JogadorAvulsoCampeonato>(
        ((avulsosData || []) as JogadorAvulsoCampeonato[]).map((j) => [
          j.id,
          j,
        ]),
      );
      ...
      const gruposMap = new Map<string, any>(
        ((gruposCampeonato || []) as any[]).map((g) => [g.id, g]),
      );
      const inscricaoPorLineId = new Map<string, any>(
        inscricoesRows
          .filter((i: any) => i.line_id)
          .map((i: any) => [i.line_id, i]),
      );

      const jogadorCampPorChave = new Map<string, JogadorCampeonato>();
      jogadoresRows.forEach((j) => {
        const insc = inscricoesRows.find(
          (i: any) => i.id === j.campeonato_equipe_id,
        );
      ...
      const equipesFormatadas: EquipeBase[] = inscricoesRows.map(
        (insc: any) => {
          if (
            insc.tipo_origem === "avulsa" ||
            (!insc.equipe_id && insc.equipe_avulsa_id)
          ) {
            ...
            return {
              ...
              grupo_nome: insc.grupo_id
                ? gruposMap.get(insc.grupo_id)?.nome || null
                : null,
      ...
      const jogadoresDiretosFormatados: JogadorFormatado[] = jogadoresRows
        ...
        .map((jc) => {
          const insc = inscricoesRows.find(
            (i: any) => i.id === jc.campeonato_equipe_id,
          );
          const perfil = jc.perfil_jogo_id
            ? perfisMap.get(jc.perfil_jogo_id)
            : null;
          const avulso = jc.jogador_avulso_id
            ? avulsosMap.get(jc.jogador_avulso_id)
            : null;
          const equipeOficial = insc?.equipe_id
            ? equipesOficiaisMap.get(insc.equipe_id)
            : null;
          const equipeAvulsa = insc?.equipe_avulsa_id
            ? equipesAvulsasMap.get(insc.equipe_avulsa_id)
            : null;
          ...
          return {
            ...
            equipe_tipo_origem: equipeOficial
              ? "app"
              : equipeAvulsa
                ? "avulsa"
                : null,
      ...
      jogadoresDiretosFormatados.forEach((jogador) => {
        ...
        if (
          !atual ||
          new Date(jogador.created_at || 0).getTime() >=
            new Date(atual.created_at || 0).getTime()
        ) {
          jogadoresDiretosUnicosMap.set(chave, jogador);
        }
      });

      const jogadoresDiretosUnicos = Array.from(
        jogadoresDiretosUnicosMap.values(),
      );

      const jogadoresFormatados: JogadorFormatado[] = jogadoresDiretosUnicos;

      const equipesComJogadores: EquipeComJogadores[] = equipesFormatadas.map(
        (eq) => ({
          ...
          jogadores: jogadoresFormatados.filter(
            (j) => j.campeonato_equipe_id === eq.inscricao_id,
          ),
        }),
      );
  ...
  const getEquipeAlvoAtual = () => {
    return equipesInscritas.find(
      (eq) => eq.inscricao_id === equipeAlvoInscricaoId,
    );
  };

  const getProximaOrdemLine = (lineId: string) => {
    const jogadoresDaLine = listaGeralJogadores.filter(
      (j) => j.line_id === lineId,
    );
  ...
  const handleRemoverJogador = async (jogador: JogadorFormatado) => {
    if (
      !confirm(
        `Deseja remover ${jogador.nome_exibicao.toUpperCase()} desta line?`,
      )
    )
      return;
  ...
  const handleUploadAvatarAvulso = async (
  ...
  const inscreverJogadorDoApp = async (perfil: PerfilJogo) => {
    try {
      ...
      const { error: garantirError } = await supabase.rpc(
  ...
  const criarJogadorAvulso = async () => {
    ...
    try {
      ...
      const { data: perfilExistente, error: perfilExistenteError } =
        await supabase
          .from("perfis_jogo")
          .select(
            "id, nick, uid_jogo, foto_capa, funcao, equipe_id, user_id, ativo",
      ...
      if (perfilExistente && perfilExistente.length > 0) {
        toast.error(
          "Já existe um perfil do app com esse ID. Use o perfil do app.",
        );
      ...
      if (!avulsoCriado?.id)
        throw new Error("Não foi possível criar o jogador avulso");
      ...
      if (jogadorAvulsoCriadoId) {
        console.error(
          "Rollback do jogador avulso executado para:",
          jogadorAvulsoCriadoId,
        );
      }
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 bg-white border border-slate-200 p-1">
        ...
        <div className="flex flex-wrap items-center gap-2 px-3 border-l border-slate-100">
          ...
          <select
            value={filtroOrigem}
            onChange={(e) =>
              setFiltroOrigem(e.target.value as "todas" | OrigemJogador)
            }
            className="bg-transparent border-none text-[9px] font-black uppercase italic outline-none text-slate-600 focus:ring-0"
          >
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      >
                        {eq.tag && (
                          <span className="opacity-50 mr-1">[{eq.tag}]</span>
                        )}{" "}
                        {eq.nome}
                      </h4>
                          ...
                          >
                            <div className="flex items-center gap-3">
                              <div className="w-7 h-7 border border-slate-200 grayscale group-hover:grayscale-0 transition-all overflow-hidden bg-white">
                                <img
                                  src={
                                    insc.foto_exibicao || "/DropZone-Logo.png"
                                  }
                              ...
                              <div className="flex flex-col">
                                <div className="flex items-center gap-2">
                                  ...
                                  <MessageShortcut
                                    ...
                                    referenciaId={
                                      insc.perfil_jogo_id || insc.id
                                    }
                                    titulo={insc.nome_exibicao || "Jogador"}
                                    avatarUrl={insc.foto_exibicao}
                                    tipo="dm"
                                  />
                                </div>
                                ...
                                <div className="flex items-center gap-2 mt-0.5">
                                  <div
                                    className={`flex items-center gap-1 text-[7px] font-bold uppercase ${style.color}`}
                                  >
                                    {style.icon}{" "}
                                    {insc.funcao_exibicao || "INDEFINIDO"}
                                  </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-2 p-3 pl-6 bg-white border-t border-slate-200">
                          <button
                            onClick={() => {
                              ...
                              setEquipeAlvoId(
                                eq.tipo_origem === "app" ? eq.id : undefined,
                              );
                              setEquipeAlvoAvulsaId(
                                eq.tipo_origem === "avulsa" ? eq.id : undefined,
                              );
            ...
            <tbody className="divide-y divide-slate-100">
              {jogadoresGeraisFiltrados.map((j) => (
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          ...
                          <MessageShortcut
                            referenciaTipo="jogador"
                            referenciaId={j.perfil_jogo_id || j.id}
                            titulo={j.nome_exibicao || "Jogador"}
                            avatarUrl={j.foto_exibicao}
                            tipo="dm"
                          />
                        </div>
                  ...
                  <td className="p-3">
                    <span
                      className={`text-[8px] font-black uppercase px-2 py-1 ${getOrigemStyle(j.origem)}`}
                    >
              ...
              {buscandoTroca ? (
                ...
                resultadosTroca.map((r) => {
                  ...
                  const jaInscrito = listaGeralJogadores.some(
                    (j) =>
                      j.id !== jogadorParaTroca.id && j.perfil_jogo_id === r.id,
                  );
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        !jaInscrito && setPerfilTrocaSelecionado(r)
                      }
