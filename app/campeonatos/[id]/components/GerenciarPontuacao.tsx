...
export default function GerenciarPontuacao({ camp, onSave }: PontuacaoProps) {
 ...
 const presetLBFF = [12, 9, 8, 7, 6, 5, 4, 3, 2, 1, 0, 0];
 ...
 const aplicarPresetLBFF = () => {
 const novoArray = Array(qtdEquipes).fill(0).map((_, i) => presetLBFF[i] ?? 0);
 setPoints(novoArray);
 };
 ...
 <button onClick={aplicarPresetLBFF} className="flex items-center justify-center gap-2 bg-zinc-100 hover:bg-white hover:text-[#2563eb] p-3 font-semibold text-[10px] uppercase transition-all">
 <Trophy size={12} /> Usar Padrão LBFF
 </button>
