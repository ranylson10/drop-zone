...
const ROMANOS = ['', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X', 'XI', 'XII', 'XIII', 'XIV', 'XV']
...
export default function GerenciarGrupos({ campeonatoId }: { campeonatoId: string }) {
  ...
  const [view, setView] = useState<'estrutura' | 'equipes' | 'trocas'>('estrutura')
  ...
  const [mapasGrupoSelecionados, setMapasGrupoSelecionados] = useState<string[]>(['Bermuda', 'Purgatório', 'Alpine', 'Kalahari'])
  const [buscaEquipe, setBuscaEquipe] = useState('')

  const [slotSelecionado, setSlotSelecionado] = useState<SlotGrupo | null>(null)

  const [slotTrocaSelecionado, setSlotTrocaSelecionado] = useState<SlotGrupo | null>(null)
  ...
  const OPCOES_MAPA = ['Bermuda', 'Purgatório', 'Alpine', 'Kalahari', 'Nexterra', 'Solara']
  ...
  const atualizarMapaGrupo = (index: number, mapa: string) => {
    setMapasGrupoSelecionados((prev) => prev.map((item, idx) => (idx === index ? mapa : item)))
  }
    ...
    try {
      const [
        ...
        supabase
          ...
          .select('id, campeonato_id, nome, slug, ordem, created_at, updated_at')
          .eq('campeonato_id', campeonatoId)
          .order('ordem', { ascending: true }),

        supabase
          .from('campeonato_grupos')
          .select('id, campeonato_id, nome, slug, fase_id, quantidade_equipes, created_at, updated_at')
          .eq('campeonato_id', campeonatoId)
        ...
        supabase
          ...
          .select('id, campeonato_id, fase_id, grupo_id, slot_numero, campeonato_equipe_id')
          .eq('campeonato_id', campeonatoId)
      ...
      const equipesFormatadas: EquipeBase[] = inscricoes.map((item: any) => {
        const tipo: 'oficial' | 'avulsa' =
          item.tipo_origem === 'avulsa' || (!item.equipe_id && item.equipe_avulsa_id)
            ? 'avulsa'
            : 'oficial'
        ...
        const chaveDuplicidade = item.equipe_id
          ...
          : item.equipe_avulsa_id
            ...
            : `inscricao:${item.id}`
        const numeroCalculado = (contagemPorEquipe.get(chaveDuplicidade) || 0) + 1
        contagemPorEquipe.set(chaveDuplicidade, numeroCalculado)
        const numeroRepeticao = Number(item.numero_vaga || numeroCalculado || 1)
        const nomeExibicao = item.nome_exibicao || montarNomeComSufixo(nomeBase, numeroRepeticao)
      ...
      const mapaEquipesBase = new Map(equipesFormatadas.map((item) => [item.inscricao_id, item]))

      const slotsFormatados: SlotGrupo[] = ((slotsData || []) as any[]).map((item) => ({
        id: item.id,
    ...
    return fases.map((fase) => {
      const gruposDaFase = grupos
        ...
        .map((grupo) => {
          ...
          const vagasPreenchidas = slotsDoGrupo.filter((slot) => !!slot.campeonato_equipe_id).length
          const vagasTotais = grupo.quantidade_equipes || slotsDoGrupo.length || 0
          const percentualPreenchimento =
            vagasTotais > 0 ? Math.round((vagasPreenchidas / vagasTotais) * 100) : 0
  ...
  const isCopa = String(campConfig?.tipo_competicao || '').toLowerCase() === 'copa'
  ...
  const equipesDisponiveisParaSlot = useMemo(() => {
    ...
    return equipesBase
      ...
      .filter((equipe) => {
        const nome = String(equipe.nome || equipe.nome_exibicao || equipe.nome_base || '')
        const nomeBase = String(equipe.nome_base || equipe.nome_exibicao || equipe.nome || '')
      ...
      .sort((a, b) => {
        const nomeA = String(a.nome_base || a.nome_exibicao || a.nome || 'Sem nome')
        const nomeB = String(b.nome_base || b.nome_exibicao || b.nome || 'Sem nome')
        const porNome = nomeA.localeCompare(nomeB, 'pt-BR')
        if (porNome !== 0) return porNome
        return Number(a.numero_repeticao || 1) - Number(b.numero_repeticao || 1)
      })
  ...
  const gruposDisponiveisTroca = useMemo(() => {
    ...
    return grupos
      .filter(
        (grupo) =>
          grupo.fase_id === slotTrocaSelecionado.fase_id && grupo.id !== slotTrocaSelecionado.grupo_id
      )
      .sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR'))
  ...
  const solicitacoesRelacionadasAoSlot = useMemo(() => {
    ...
    return solicitacoes.filter((solicitacao) => {
      ...
      if (solicitacao.solicitante_campeonato_equipe_id === slotTrocaSelecionado.campeonato_equipe_id)
        return true
  ...
  const solicitacoesPorGrupo = useMemo(() => {
    ...
    for (const solicitacao of solicitacoes) {
      ...
      if (solicitacao.grupo_desejado_id) {
        if (!mapa.has(solicitacao.grupo_desejado_id)) mapa.set(solicitacao.grupo_desejado_id, [])
  ...
  const handleCriarFase = async () => {
    ...
    const proximaOrdem =
      fases.length > 0 ? Math.max(...fases.map((fase) => Number(fase.ordem || 0))) + 1 : 1
  ...
  const handleCriarGrupo = async () => {
    ...
    try {
      ...
      if (isCopa) {
        ...
        if (updateGrupoError) {
          console.error('Aviso ao salvar dados do grupo na copa:', updateGrupoError)
        }
      ...
      setFasesAbertas((prev) =>
        prev.includes(faseSelecionadaParaGrupo) ? prev : [...prev, faseSelecionadaParaGrupo]
      )
  ...
  const handleExcluirFase = async (fase: FaseComGrupos) => {
    ...
    try {
      const { error } = await supabase.from('campeonato_fases').delete().eq('id', fase.id)
      if (error) throw error
  ...
  const handleAdicionarEquipeNoSlot = async (campeonatoEquipeId: string) => {
    ...
    const vagaSelecionada = equipesBase.find((equipe) => equipe.inscricao_id === campeonatoEquipeId)
    const nomeVaga = vagaSelecionada?.nome_exibicao || vagaSelecionada?.nome || 'Equipe'
    ...
    try {
      ...
      if (vagaJaUsadaNaFase) {
        throw new Error('Esta vaga específica já está em outro slot desta fase.')
      }
      ...
      if (slotError) {
        console.error('Erro ao atualizar campeonato_grupo_slots:', JSON.stringify(slotError, null, 2))
        throw slotError
      }
      ...
      if (equipeError) {
        console.error('Erro ao atualizar campeonato_equipes:', JSON.stringify(equipeError, null, 2))
      ...
      if (!campeonatoIdAlvo || !faseIdAlvo) {
        ...
        if (jogosDoGrupoError) {
          console.error('Erro ao buscar jogos do grupo para sincronizar súmula:', JSON.stringify(jogosDoGrupoError, null, 2))
          ...
          if (jogoEquipesError) {
            console.error('Erro ao sincronizar jogo_equipes:', JSON.stringify(jogoEquipesError, null, 2))
          }
        }
      }
      ...
      console.error('Erro ao adicionar equipe no slot:', JSON.stringify(error, null, 2))
      toast.error(error?.message || error?.details || error?.hint || 'Erro ao adicionar equipe ao slot')
    } finally {
      setSalvando(false)
    }
  }
  ...
  const handleRemoverEquipeDoSlot = async (slot: SlotGrupo) => {
    ...
    const nomeVaga = slot.equipe?.nome_exibicao || slot.equipe?.nome || 'esta equipe'
    ...
    try {
      ...
      if (slotError) {
        console.error('Erro ao limpar campeonato_grupo_slots:', JSON.stringify(slotError, null, 2))
        throw slotError
      }
      ...
      if (jogoEquipesDeleteError) {
        console.error('Erro ao remover vínculo em jogo_equipes:', JSON.stringify(jogoEquipesDeleteError, null, 2))
      ...
      if (!aindaUsadaEmOutroSlot) {
        ...
        if (equipeError) {
          console.error('Erro ao limpar campeonato_equipes:', JSON.stringify(equipeError, null, 2))
          throw equipeError
        }
      }
      ...
      console.error('Erro ao remover equipe do slot:', JSON.stringify(error, null, 2))
      toast.error(error?.message || error?.details || error?.hint || 'Erro ao remover equipe do slot')
    } finally {
      setSalvando(false)
    }
  }
  ...
  const criarSolicitacaoTroca = async () => {
    if (!slotTrocaSelecionado?.campeonato_equipe_id || !slotTrocaSelecionado.equipe) return
    ...
    try {
      const { error } = await supabase.rpc('criar_solicitacao_troca_grupo', {
        p_campeonato_id: campeonatoId,
        p_solicitante_campeonato_equipe_id: slotTrocaSelecionado.campeonato_equipe_id,
        p_grupo_desejado_id: aceitaQualquerGrupo ? null : grupoDesejadoTroca || null,
  ...
  const aceitarSolicitacaoTroca = async (solicitacao: SolicitacaoTroca) => {
    ...
    try {
      ...
      const { error } = await supabase.rpc('aceitar_troca_grupo', {
        ...
        p_equipe_destino_campeonato_equipe_id: slotTrocaSelecionado.campeonato_equipe_id,
      ...
      const slotSolicitante = slots.find(
        (slot) => slot.campeonato_equipe_id === solicitacao.solicitante_campeonato_equipe_id
      )
      const slotDestino = slots.find(
        (slot) => slot.campeonato_equipe_id === slotTrocaSelecionado.campeonato_equipe_id
      )
      ...
      if (slotSolicitante && slotDestino) {
        ...
        const { error: e2 } = await supabase
          .from('campeonato_grupo_slots')
          .update({ campeonato_equipe_id: slotSolicitante.campeonato_equipe_id })
  ...
  const formatarTipoValidacao = (tipo: string | null) => {
    if (tipo === 'mesma_fase_e_mesmas_quedas') return 'Mesma fase + mesmas quedas'
  ...
  const formatarAgendaGrupo = (grupo: GrupoComSlots) => {
    const anyGrupo = grupo as any
    const data = anyGrupo.data_jogo ? new Date(anyGrupo.data_jogo).toLocaleDateString('pt-BR') : null
    ...
    <div className="space-y-4">
        ...
        <div className="flex gap-1 flex-wrap">
          <button
            ...
            className={`px-4 py-2 text-[9px] font-black italic uppercase transition-all flex items-center gap-2 ${
              view === 'estrutura' ? 'bg-slate-900 text-[#7cfc00]' : 'text-slate-400 hover:bg-slate-50'
            }`}
          >
          ...
          <button
            ...
            className={`px-4 py-2 text-[9px] font-black italic uppercase transition-all flex items-center gap-2 ${
              view === 'trocas' ? 'bg-slate-900 text-[#7cfc00]' : 'text-slate-400 hover:bg-slate-50'
            }`}
          >
      ...
      {campConfig && (
        <div className="bg-white border border-slate-200 p-4 flex flex-wrap gap-3 text-[10px] font-black uppercase italic">
          <span className={`px-3 py-1 border ${campConfig.permitir_troca_grupos ? 'bg-[#7cfc00] text-black border-black' : 'bg-zinc-100 text-zinc-500 border-zinc-200'}`}>
            Troca de grupos: {campConfig.permitir_troca_grupos ? 'ATIVA' : 'DESATIVADA'}
          </span>
          <span className="px-3 py-1 border border-zinc-200 bg-zinc-50 text-zinc-700">
            Troca livre: {campConfig.permitir_troca_grupos_livre ? 'SIM' : 'NÃO'}
          </span>
          <span className="px-3 py-1 border border-zinc-200 bg-zinc-50 text-zinc-700">
            Troca genérica: {campConfig.permitir_troca_grupos_generica ? 'SIM' : 'NÃO'}
          </span>
          <span className="px-3 py-1 border border-zinc-200 bg-zinc-50 text-zinc-700">
            Aprovação: {campConfig.exigir_aprovacao_organizacao_troca_grupo ? 'OBRIGATÓRIA' : 'LIVRE'}
          </span>
          <span className="px-3 py-1 border border-zinc-200 bg-zinc-50 text-zinc-700">
        ...
        <div className="grid grid-cols-1 gap-[2px] bg-slate-200 border border-slate-200 shadow-sm">
          {estrutura.length === 0 ? (
            ...
            estrutura.map((fase) => {
              ...
              const genericasFase = solicitacoesGenericasPorFase.get(fase.id) || []
                ...
                <div key={fase.id} className="bg-white">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-black uppercase italic text-white">{fase.nome}</span>
                    ...
                    <div className="grid grid-cols-1 gap-[2px] bg-slate-100">
                      {fase.grupos.length === 0 ? (
                        ...
                        fase.grupos.map((grupo) => {
                          ...
                          const solicitacoesDoGrupo = solicitacoesPorGrupo.get(grupo.id) || []
                                ...
                                <div className="flex items-start justify-between gap-4 px-5 py-4">
                                  <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-3 flex-wrap">
                                      ...
                                      <span className="text-[10px] font-black uppercase italic">{grupo.nome}</span>
                                      <MessageShortcut
                                      ...
                                      <span className="text-[8px] font-black uppercase italic text-slate-400">
                                        {grupo.vagas_preenchidas}/{grupo.vagas_totais} slots
                                      </span>
                                    ...
                                    {isCopa ? (
                                      <div className="mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4">
                                        <div className="border border-slate-200 bg-white px-3 py-2">
                                          <div className="text-[8px] font-black uppercase italic text-slate-400">Agenda</div>
                                        ...
                                        <div className="border border-slate-200 bg-white px-3 py-2">
                                          <div className="text-[8px] font-black uppercase italic text-slate-400">Partidas</div>
                                          <div className="mt-1 text-[10px] font-black uppercase italic text-slate-800">
                                            {(grupo as any).qtd_quedas || 0} partida(s)
                                          </div>
                                        </div>

                                        <div className="border border-slate-200 bg-white px-3 py-2">
                                          <div className="text-[8px] font-black uppercase italic text-slate-400">Classificam</div>
                                          <div className="mt-1 text-[10px] font-black uppercase italic text-slate-800">
                                            {(grupo as any).classificam || 0} equipe(s)
                                          </div>
                                        </div>

                                        <div className="border border-slate-200 bg-white px-3 py-2">
                                          <div className="text-[8px] font-black uppercase italic text-slate-400">Mapas</div>
                              ...
                              {abertoGrupo && (
                                <div className="p-4">
                                  <div className="space-y-2">
                                    ...
                                    const slotAberto = slotsAbertos.includes(slot.id)
                                    ...
                                    return (
                                      ...
                                      >
                                        <div
                                          role="button"
                                          tabIndex={0}
                                          onClick={() => slot.equipe && toggleSlot(slot.id)}
                                          onKeyDown={(e) => {
                                              ...
                                              <>
                                                ...
                                                <div className="min-w-0">
                                                  <div className="flex min-w-0 items-center gap-2">
                                                    ...
                                                    {slot.equipe.equipe_id && (
                                                      <MessageShortcut referenciaTipo="equipe" referenciaId={slot.equipe.equipe_id} titulo={slot.equipe.nome || "Equipe"} avatarUrl={slot.equipe.logo_url} tipo="equipe" />
                                                    )}
                                                  </div>
                                                  ...
                                                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                                                    ...
                                                    <span
                                                      className={`text-[7px] font-black uppercase px-1.5 py-0.5 ${
                                                        slot.equipe.tipo_origem === 'oficial'
                                            ...
                                            {slot.equipe ? (
                                              <>
                                                <button
                                                  onClick={(e) => {
                                                    e.stopPropagation()
                                                    handleRemoverEquipeDoSlot(slot)
                                                  }}
                                                ...
                                                <button
                                                  ...
                                                  title="Abrir ações"
                                                >
                                                  {slotAberto ? <ChevronDown size={14} /> : <ChevronRight size={14} />}
                                                </button>
                                              </>
                                            ) : (
                                              <button
                                                onClick={() => abrirSelecaoEquipe(slot)}
        ...
        <div className="bg-white border border-slate-200 shadow-sm p-5">
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
            {equipesBase
              ...
              .sort((a, b) => a.nome.localeCompare(b.nome, 'pt-BR'))
              .map((equipe) => {
                const grupo = equipe.grupo_id ? mapaGrupos.get(equipe.grupo_id) : null

                return (
                  <div key={equipe.inscricao_id} className="border border-slate-200 bg-slate-50 p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        <div className="flex min-w-0 items-center gap-2">
                          ...
                          {equipe.equipe_id && (
                            <MessageShortcut referenciaTipo="equipe" referenciaId={equipe.equipe_id} titulo={equipe.nome || "Equipe"} avatarUrl={equipe.logo_url} tipo="equipe" />
                          )}
                        </div>
      ...
      {view === 'trocas' && (
        <div className="bg-white border border-slate-200 shadow-sm">
          <div className="p-5 border-b border-slate-200 flex items-center justify-between gap-3">
            <div>
              <h3 className="text-[11px] font-black uppercase italic">Solicitações de troca</h3>
              <p className="mt-1 text-[9px] font-black uppercase italic text-slate-400">
                Mesmo fase sempre • pontos corridos exige mesma quantidade de quedas na validação final
              </p>
            </div>
          ...
          {solicitacoes.length === 0 ? (
            ...
            <div className="divide-y divide-slate-200">
              {solicitacoes.map((solicitacao) => {
                const equipeSolicitante = mapaEquipes.get(solicitacao.solicitante_campeonato_equipe_id)
                ...
                return (
                  <div key={solicitacao.id} className="p-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                    <div>
                      ...
                      {solicitacao.observacao && (
                        <p className="mt-2 text-[10px] text-slate-500">{solicitacao.observacao}</p>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="w-full max-w-lg border-2 border-black bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <div className="flex items-center justify-between border-b-2 border-black bg-black px-5 py-4">
              <div>
                <h3 className="text-sm font-black uppercase italic text-[#7cfc00]">Nova fase</h3>
      ...
      {showModalGrupo && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4">
          <div className="w-full max-w-xl border-2 border-black bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <div className="flex items-center justify-between border-b-2 border-black bg-black px-5 py-4">
              <div>
                <h3 className="text-sm font-black uppercase italic text-[#7cfc00]">Novo grupo</h3>
                <p className="mt-1 text-[10px] font-black uppercase italic text-zinc-400">
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      {Array.from({ length: Number(quantidadeQuedasGrupo || 0) }).map((_, index) => (
                        <div key={`mapa-partida-${index}`} className="border-2 border-black bg-white p-3">
                          ...
                          <select
                            ...
                            onChange={(e) => atualizarMapaGrupo(index, e.target.value)}
                            className="w-full border-2 border-black bg-white px-4 py-3 text-[11px] font-black uppercase outline-none"
                          >
          ...
          <div className="w-full max-w-3xl border-2 border-black bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <div className="flex items-center justify-between border-b-2 border-black bg-black px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-[10px] font-black uppercase italic text-zinc-400">
                  {mapaGrupos.get(slotSelecionado.grupo_id)?.nome || 'Grupo'} • Slot {slotSelecionado.slot_numero}
                </p>
              </div>
                  ...
                  <div className="divide-y divide-slate-200">
                    {equipesDisponiveisParaSlot.map((equipe) => (
                      <button
                        ...
                        onClick={() => handleAdicionarEquipeNoSlot(equipe.inscricao_id)}
                        disabled={salvando}
                        ...
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                              ...
                              >
                                {equipe.tipo_origem === 'oficial' ? 'APP' : 'AVULSA'}
                              </span>
                            </div>
                          </div>
                        </div>
          ...
          <div className="w-full max-w-4xl border-2 border-black bg-white shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <div className="flex items-center justify-between border-b-2 border-black bg-black px-5 py-4">
              <div>
                ...
                <p className="mt-1 text-[10px] font-black uppercase italic text-zinc-400">
                  {slotTrocaSelecionado.equipe.nome} • {mapaGrupos.get(slotTrocaSelecionado.grupo_id)?.nome || 'Grupo atual'}
                </p>
              </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[9px] font-black uppercase italic text-slate-400">
                          Fase {fases.find((f) => f.id === slotTrocaSelecionado.fase_id)?.nome || '---'} • Grupo {mapaGrupos.get(slotTrocaSelecionado.grupo_id)?.nome || '---'}
                        </p>
                      </div>
                    </div>
                  </div>
                ...
                <div className="space-y-3 max-h-[520px] overflow-y-auto">
                  {solicitacoesRelacionadasAoSlot.length === 0 ? (
                    ...
                    solicitacoesRelacionadasAoSlot.map((solicitacao) => {
                      ...
                      return (
                        <div key={solicitacao.id} className="border border-slate-200 bg-slate-50 p-4">
                          <div className="flex items-center justify-between gap-3">
                            ...
                            {ehMinhaSolicitacao ? (
                              ...
                              <button
                                onClick={() => aceitarSolicitacaoTroca(solicitacao)}
                          ...
                          {solicitacao.observacao && (
                            <p className="mt-3 text-[10px] text-slate-500">{solicitacao.observacao}</p>
                          )}

                          <div className="mt-3 text-[8px] font-black uppercase italic text-slate-400">
                            Quedas do solicitante: {solicitacao.quedas_jogadas_solicitante}
                          </div>
                        </div>
                      )
                    })
            ...
            {campConfig?.exigir_aprovacao_organizacao_troca_grupo && (
              <div className="border-t border-slate-200 bg-amber-50 px-5 py-3 flex items-center gap-2 text-[9px] font-black uppercase italic text-amber-700">
                <Shield size={13} />
                Este campeonato exige aprovação da organização para concluir a troca.
              </div>
            )}
          </div>
        </div>
