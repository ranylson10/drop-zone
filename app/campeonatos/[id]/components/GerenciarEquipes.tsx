  ...
  return (
    <div className="border border-amber-200 bg-amber-50 p-5 text-sm font-semibold text-amber-800">
      <div className="text-[11px] font-black uppercase tracking-[0.18em] text-amber-700">
        Edição de equipes bloqueada
      </div>
      <p className="mt-2 leading-6">
        Você pode acessar a aba, mas adicionar, trocar, confirmar pagamento ou
        remover equipes é permitido apenas para o dono do campeonato ou
        ajudantes autorizados.
      </p>
    </div>
  );
}
...
export default function GerenciarEquipes({
  ...
  const [equipesAppEncontradas, setEquipesAppEncontradas] = useState<
    EquipeOficial[]
  >([]);
  const [equipeAppSelecionada, setEquipeAppSelecionada] =
  ...
  const [inscricaoTroca, setInscricaoTroca] = useState<EquipeLista | null>(
    null,
  );
  const [buscaTroca, setBuscaTroca] = useState("");
  const [equipesTrocaEncontradas, setEquipesTrocaEncontradas] = useState<
    EquipeOficial[]
  >([]);
 ...
 `,
      ...
      if (idsAvulsas.length > 0) {
        const { data: avulsasData, error: avulsasError } = await supabase
          ...
          .select(
            "id, campeonato_id, nome, tag, logo_url, criada_por, equipe_oficial_id",
          )
          .in("id", idsAvulsas);
      ...
      const listaNormalizada: EquipeLista[] = inscricoesRows
        .map((item) => {
          ...
          const tipo: "app" | "avulsa" =
            item.tipo_origem === ORIGEM_AVULSA_BANCO ||
          ...
          const chaveRepeticao = oficial?.id
            ...
            : avulsa?.equipe_oficial_id
              ? `app:${avulsa.equipe_oficial_id}`
              : `nome:${nome.trim().toLowerCase()}`;
          const repeticao_numero =
            (contadorRepeticoes.get(chaveRepeticao) || 0) + 1;
          contadorRepeticoes.set(chaveRepeticao, repeticao_numero);
          const sufixosRomanos = [
            "",
            "",
            "II",
            "III",
          ...
          const sufixo =
            repeticao_numero <= 10
              ? sufixosRomanos[repeticao_numero]
              : String(repeticao_numero);
          const nome_exibicao_calculado =
            repeticao_numero > 1 ? `${nome} ${sufixo}` : nome;
          ...
          return {
            ...
            origem_inscricao:
              item.origem_inscricao === "site"
                ? ("site" as const)
                : ("organizador" as const),
            status_pagamento:
              item.status_pagamento ||
              (item.origem_inscricao === "site" ? "pago" : "manual"),
        ...
        .sort(
          (a, b) =>
            ...
            a.repeticao_numero - b.repeticao_numero,
        );
        ...
        tipoFiltro === "todas"
          ...
          : tipoFiltro === "site"
            ...
            : tipoFiltro === "organizador"
              ? eq.origem_inscricao === "organizador" &&
                eq.status_pagamento !== "agendado"
  ...
  const handleUploadLogoAvulsa = async (
  ...
  const dadosControleVaga = async () => {
    ...
    return {
      ...
      agendada_para:
        statusNovaVaga === "agendada" && dataAgendamento
          ? dataAgendamento
          : null,
  ...
  const adicionarEquipeAvulsa = async () => {
    ...
    try {
      ...
      if (!user?.id) {
        throw new Error(
          "Você precisa estar logado para criar equipe temporária.",
        );
      }
      ...
      if (!avulsaCriada?.id)
        throw new Error("Não foi possível criar a equipe temporária.");
      ...
      const { error: erroInscricao } = await supabase.rpc(
        "fn_adicionar_vaga_organizador",
        {
          p_campeonato_id: campeonatoId,
          p_equipe_id: null,
      ...
      if (avulsaCriadaId) {
        console.error(
          "Falha após criar avulsa, tentativa de rollback executada:",
          avulsaCriadaId,
        );
      }
  ...
  const handleRemoverEquipe = async (item: EquipeLista) => {
    ...
    try {
      ...
      alert(
        error?.message ||
          "Erro ao remover equipe. Se já tiver jogado, a remoção fica bloqueada.",
      );
    } finally {
      setSalvando(false);
    }
  ...
  const handleCancelarVagaAgendada = async (item: EquipeLista) => {
    ...
    try {
      ...
      alert(
        error?.message ||
          "Erro ao cancelar vaga agendada. Se já tiver jogado, o cancelamento fica bloqueado.",
      );
    } finally {
      setSalvando(false);
    }
                  ...
                  <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-2">
                        ...
                        {item.equipe_oficial_id_vinculada &&
              ...
              <div className="grid grid-cols-1 gap-4 border border-zinc-200 bg-zinc-50 p-4 md:grid-cols-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setStatusNovaVaga(e.target.value as "ativa" | "agendada")
                    }
                  ...
                  <div className="max-h-72 overflow-auto border border-zinc-200">
                    {equipesAppEncontradas.length === 0 ? (
                      ...
                      equipesAppEncontradas.map((eq) => {
                        ...
                        const vezesInscrita = equipes.filter(
                          (item) => item.equipe_id === eq.id,
                        ).length;
                          ...
                          >
                            ...
                            <div className="text-[9px] font-semibold uppercase text-zinc-500">
                              {selecionada
                                ? "Selecionada"
                                : vezesInscrita > 0
                                  ? `Adicionar de novo (${vezesInscrita + 1}ª vaga)`
                                  : "Selecionar"}
                            </div>
                          </button>
                        );
                ...
                {equipesTrocaEncontradas.length === 0 ? (
                  ...
                  equipesTrocaEncontradas.map((eq) => {
                    ...
                    const jaInscrita = equipes.some(
                      (item) =>
                        item.inscricao_id !== inscricaoTroca.inscricao_id &&
                        item.equipe_id === eq.id,
                    );
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          !jaInscrita && setEquipeTrocaSelecionada(eq)
                        }
                        className={`flex w-full items-center justify-between border-b border-zinc-200 px-4 py-3 text-left last:border-b-0 ${
                          selecionada ? "bg-[#ecffcf]" : "bg-white"
