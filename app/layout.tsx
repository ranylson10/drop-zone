  ...
  useEffect(() => {
    ...
    async function carregarCarteira() {
      try {
        ...
        if (!uid) {
          if (ativo) setSaldoCarteira(0)
          return
        }

        const { data: wallet } = await supabase.from('wallet_saldo').select('saldo').eq('user_id', uid).maybeSingle()
        if (ativo) setSaldoCarteira(Number(wallet?.saldo || 0))
      } catch {
        if (ativo) setSaldoCarteira(0)
      }
    }
      ...
      <div className="flex h-16 items-center justify-between gap-3 px-3 md:px-5">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="hidden min-w-0 md:block">
            ...
            <div className="truncate text-[14px] font-bold text-slate-950">Competitivo, social, carteira e alertas</div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 md:gap-3">
          ...
          {usuarioLogado ? (
            <button
              ...
              className="hidden items-center gap-3 border border-[#2563eb]/20 bg-white px-3 py-2 transition hover:border-[#2563eb]/50 hover:bg-slate-50 md:flex"
            >
              <div className="flex h-9 w-9 items-center justify-center border border-[#2563eb]/25 bg-[#2563eb]/10 text-[#2563eb]">
                <Wallet size={16} />
              </div>
              <div className="text-left leading-none">
                <div className="text-[9px] font-black uppercase tracking-[0.18em] text-slate-500">Carteira</div>
                <div className="mt-1 text-[12px] font-bold uppercase text-slate-950">{formatarDinheiro(saldoCarteira)}</div>
              </div>
            </button>
          ) : null}
