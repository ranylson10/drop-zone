  ...
  <div className="space-y-5">
   <div className={classNamePainel(true)}>
    <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
     <div>
      <div className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#142340]">Lista de confrontos</div>
      <p className="mt-2 text-sm font-semibold text-zinc-500">Agenda no estilo futebol: time A x time B, horário, MD e resultado.</p>
     </div>
     <button type="button" onClick={carregar} className="inline-flex h-10 items-center gap-2 border border-zinc-200 bg-[#f3f3ef] px-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#142340] hover:bg-[#eceee5]"><RefreshCcw size={14} /> Atualizar</button>
    </div>
    {loading ? <div className="border border-dashed border-zinc-200 bg-[#f8f8f5] px-6 py-10 text-center text-sm font-semibold text-zinc-500">Carregando confrontos...</div> : confrontosPorFase.length === 0 ? <div className="border border-dashed border-zinc-200 bg-[#f8f8f5] px-6 py-10 text-center text-sm font-semibold text-zinc-500">Nenhuma fase criada ainda.</div> : (
     <div className="space-y-5">
      {confrontosPorFase.map(({ fase, jogos }) => {
       const IconeFase = iconeTipo(fase.tipo_fase)
       return <div key={fase.id} className="border border-zinc-200 bg-[#f8f8f5]">
        <div className="flex flex-wrap items-center gap-3 border-b border-zinc-200 bg-[#eceee5] px-4 py-3"><IconeFase size={15} /><div className="text-sm font-semibold uppercase tracking-[0.08em] text-[#142340]">{fase.nome || 'Fase'}</div><span className={`border border-zinc-200 px-2 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] ${getTipoBadgeClass(fase.tipo_fase)}`}>{getTipoLabel(fase.tipo_fase)}</span></div>
        <div className="divide-y divide-zinc-200">
         {jogos.length === 0 ? <div className="bg-white px-5 py-8 text-center text-sm font-semibold text-zinc-500">Nenhum confronto criado nessa fase.</div> : jogos.map((jogo: any) => {
          const equipe1 = jogo.equipes?.[0] || null
          const equipe2 = jogo.equipes?.[1] || null
          const r = getResumoJogo(jogo)
          const vencedor = r.vitoriasA > r.vitoriasB ? getEquipeNome(equipe1) : r.vitoriasB > r.vitoriasA ? getEquipeNome(equipe2) : null
          return <div key={jogo.id} className="bg-white p-4">
           <div className="grid gap-4 xl:grid-cols-[1fr,130px,1fr,220px] xl:items-center">
            <div className="flex items-center gap-3">
             <div className="flex h-12 w-12 items-center justify-center overflow-hidden border border-zinc-200 bg-white">{getEquipeLogo(equipe1) ? <img src={getEquipeLogo(equipe1) as string} alt="" className="h-full w-full object-cover" /> : <span className="text-lg font-black text-zinc-500">{getEquipeNome(equipe1).charAt(0).toUpperCase()}</span>}</div>
             <div className="min-w-0"><div className="truncate text-base font-black uppercase text-[#142340]">{getEquipeNome(equipe1)}</div><div className="text-[11px] font-semibold uppercase text-zinc-500">{getEquipeTag(equipe1) || 'Sem tag'}</div></div>
            </div>
            <div className="text-center"><div className="text-[28px] font-black text-[#142340]">{r.vitoriasA} x {r.vitoriasB}</div><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-zinc-500">MD{r.md}</div></div>
            <div className="flex items-center gap-3 xl:flex-row-reverse xl:text-right">
   ...
   <div className="grid gap-5 xl:grid-cols-2">
    ...
    <div className={classNamePainel()}>
     <div className="mb-4 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#142340]">Montar confrontos</div>
     <div className="grid gap-3"><select value={faseSelecionada} onChange={(e) => setFaseSelecionada(e.target.value)} className="h-11 border border-zinc-200 bg-white px-3 text-sm font-semibold outline-none"><option value="">Selecione a fase</option>{fases.map((fase) => <option key={fase.id} value={fase.id}>{fase.nome || 'Fase'} • {getTipoLabel(fase.tipo_fase)}</option>)}</select><select value={tipoConfronto} onChange={(e) => setTipoConfronto(e.target.value)} className="h-11 border border-zinc-200 bg-white px-3 text-sm font-semibold outline-none"><option value="mata_mata">Mata-mata</option><option value="dupla_eliminacao">Dupla eliminação</option><option value="pontos_corridos">Pontos corridos</option></select><input value={nomeConfronto} onChange={(e) => setNomeConfronto(e.target.value)} placeholder="Nome do confronto (opcional)" className="h-11 border border-zinc-200 bg-white px-3 text-sm font-semibold outline-none" />
      {tipoConfronto !== 'pontos_corridos' && <><div className="grid gap-3 md:grid-cols-2"><select value={equipeA} onChange={(e) => { setEquipeA(e.target.value); if (e.target.value === equipeB) setEquipeB('') }} className="h-11 border border-zinc-200 bg-white px-3 text-sm font-semibold outline-none"><option value="">Equipe A</option>{opcoesEquipeA.map((equipe) => <option key={equipe.id} value={equipe.id}>{getEquipeNome(equipe)}{getEquipeTag(equipe) ? ` • ${getEquipeTag(equipe)}` : ''}</option>)}</select><select value={equipeB} onChange={(e) => setEquipeB(e.target.value)} className="h-11 border border-zinc-200 bg-white px-3 text-sm font-semibold outline-none"><option value="">Equipe B</option>{opcoesEquipeB.map((equipe) => <option key={equipe.id} value={equipe.id}>{getEquipeNome(equipe)}{getEquipeTag(equipe) ? ` • ${getEquipeTag(equipe)}` : ''}</option>)}</select></div><div className="grid gap-3 md:grid-cols-2"><PreviewEquipeSelecionada titulo="Lado A" equipe={equipesDisponiveisDaFase.find((item) => item.id === equipeA) || null} /><PreviewEquipeSelecionada titulo="Lado B" equipe={equipesDisponiveisDaFase.find((item) => item.id === equipeB) || null} /></div></>}
      ...
      {tipoConfronto === 'pontos_corridos' ? <button type="button" onClick={gerarPontosCorridos} disabled={salvando} className="inline-flex h-11 items-center justify-center gap-2 border border-zinc-200 bg-white px-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#142340] hover:bg-[#2563eb] disabled:opacity-60"><ListOrdered size={14} /> Gerar todos contra todos</button> : <div className="grid gap-3 md:grid-cols-2"><button type="button" onClick={criarConfrontoIndividual} disabled={salvando} className="inline-flex h-11 items-center justify-center gap-2 border border-zinc-200 bg-white px-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#142340] hover:bg-[#2563eb] disabled:opacity-60"><Swords size={14} /> Criar confronto</button><button type="button" onClick={gerarConfrontosAutomaticamente} disabled={salvando} className="inline-flex h-11 items-center justify-center gap-2 border border-zinc-200 bg-[#f3f3ef] px-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#142340] hover:bg-[#dff3ff] disabled:opacity-60"><Shuffle size={14} /> Gerar automático</button></div>}
     </div>
    </div>
   </div>
  </div>
