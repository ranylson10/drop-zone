...
function formatarDataCurta(jogo?: JogoItem | null) {
 ...
 if (jogo.data_hora) {
  try {
   return new Date(jogo.data_hora).toLocaleDateString('pt-BR')
  } catch {
   return '-'
  }
 }
 if (jogo.data_inicio) {
  try {
   return new Date(jogo.data_inicio).toLocaleDateString('pt-BR')
  } catch {
   return '-'
  }
 }
...
function formatarHoraCurta(jogo?: JogoItem | null) {
 ...
 if (jogo.data_hora) {
  try {
   return new Date(jogo.data_hora).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
  } catch {
   return '-'
  }
 }
 if (jogo.data_inicio) {
  try {
   return new Date(jogo.data_inicio).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
  } catch {
   return '-'
  }
 }
...
function obterResultadoFinal(jogo: JogoItem) {
...
export default function GerenciarConfrontos({ campeonatoId }: { campeonatoId: string }) {
 ...
 <div>
 <div className="text-[12px] font-black uppercase tracking-[0.14em] text-[#142340]">
 Lista de confrontos
 </div>
 <p className="mt-2 text-sm font-semibold text-zinc-500">
 Confrontos em linha única: equipe A x equipe B, data, horário, MD e quedas.
 </p>
 </div>
 ...
 type="button"
 onClick={carregar}
 className="inline-flex h-11 items-center justify-center gap-2 border border-zinc-200 bg-white px-4 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#142340] hover:bg-[#f3f3ef]"
 >
 <RefreshCcw size={14} />
 Atualizar
 </button>
 ...
 {loading ? (
 <div className="border border-dashed border-zinc-200 bg-white p-8 text-center text-sm font-semibold text-zinc-500">
 ...
 <div className="flex items-center gap-2 text-sm font-black uppercase tracking-[0.08em] text-[#142340]">
 <IconeFase size={15} />
 {fase.nome || 'Fase'}
 </div>
 <span className={`inline-flex items-center px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] text-[#142340] ${getTipoBadgeClass(fase.tipo_fase)}`}>
 {getTipoLabel(fase.tipo_fase)}
 </span>
 </div>

 <div className="space-y-2 p-3">
 {jogosDaFase.map((jogo: any, index: number) => {
 const equipe1 = jogo.equipes?.[0] || null
 const equipe2 = jogo.equipes?.[1] || null
 const tipoJogo = jogo.configuracao?.tipo || fase.tipo_fase
 ...
 const resultado = obterResultadoFinal(jogo)

 return (
 <div
 key={jogo.id}
 className="grid items-center gap-3 rounded-sm border border-[#101827] bg-[#121b2a] px-4 py-3 text-white shadow-[3px_3px_0px_0px_rgba(0,0,0,0.35)] lg:grid-cols-[42px_minmax(280px,1fr)_220px_70px_90px_auto]"
 >
 <div className="text-center text-[12px] font-black text-zinc-300">
 {String(jogo.ordem || jogo.numero || index + 1).padStart(2, '0')}
 </div>

 <div className="flex min-w-0 items-center justify-center gap-4">
 <div className="flex min-w-0 flex-1 items-center justify-end gap-3">
 <div className="min-w-0 text-right">
 <div className="truncate text-sm font-black uppercase">{getEquipeNome(equipe1)}</div>
 <div className="truncate text-[10px] font-bold uppercase tracking-[0.12em] text-zinc-400">{getEquipeTag(equipe1) || 'Sem tag'}</div>
 </div>
 <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-white">
 {getEquipeLogo(equipe1) ? (
 <img src={getEquipeLogo(equipe1) as string} alt={getEquipeNome(equipe1)} className="h-full w-full object-cover" />
 ) : (
 <span className="text-base font-black text-[#142340]">{getEquipeNome(equipe1).charAt(0).toUpperCase()}</span>
 )}
 </div>
 </div>

 <div className="shrink-0 rounded bg-black/25 px-4 py-2 text-center text-xl font-black leading-none">
 {resultado || 'X'}
 </div>

 <div className="flex min-w-0 flex-1 items-center justify-start gap-3">
 <div className="flex h-11 w-11 shrink-0 items-center justify-center overflow-hidden rounded-full border border-white/10 bg-white">
 {getEquipeLogo(equipe2) ? (
 <img src={getEquipeLogo(equipe2) as string} alt={getEquipeNome(equipe2)} className="h-full w-full object-cover" />
 ) : (
 <span className="text-base font-black text-[#142340]">{getEquipeNome(equipe2).charAt(0).toUpperCase()}</span>
 )}
 </div>
 <div className="min-w-0">
 <div className="truncate text-sm font-black uppercase">{getEquipeNome(equipe2)}</div>
 <div className="truncate text-[10px] font-bold uppercase tracking-[0.12em] text-zinc-400">{getEquipeTag(equipe2) || 'Sem tag'}</div>
 </div>
 </div>
 </div>

 <div className="flex flex-wrap items-center justify-center gap-4 text-[12px] font-semibold text-zinc-200 lg:justify-start">
 <span className="inline-flex items-center gap-2"><CalendarDays size={15} />{formatarDataCurta(jogo)}</span>
 <span className="inline-flex items-center gap-2"><Clock3 size={15} />{formatarHoraCurta(jogo)}</span>
 </div>

 <div className="text-center text-[12px] font-black uppercase text-white">
 MD{melhorDeTexto}
 </div>

 <div className="text-center text-[12px] font-semibold text-zinc-200">
 {qtdQuedas} queda{Number(qtdQuedas) === 1 ? '' : 's'}
 </div>
 ...
 className="inline-flex h-9 w-9 items-center justify-center border border-white/10 bg-white/5 text-white hover:bg-white/10"
 >
 <Pencil size={15} />
 </button>
 <button
 ...
 className="inline-flex h-9 w-9 items-center justify-center border border-red-400/30 bg-red-500/10 text-red-300 hover:bg-red-500/20"
 >
 <Trash2 size={15} />
 </button>
 </div>
 ...
 className="h-11 w-full border border-zinc-200 bg-white px-4 text-sm font-semibold outline-none"
 />
 ...
 onClick={criarFase}
 disabled={salvando}
 className="inline-flex h-11 w-full items-center justify-center gap-2 border border-zinc-200 bg-white px-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#142340] hover:bg-[#2563eb] disabled:opacity-60"
 >
 <Plus size={14} />
 Criar fase
 </button>
 ...
 <p className="mt-2 text-sm font-semibold text-zinc-500">
 Selecione as equipes, data, horário, MD e quantidade de quedas.
 </p>
 </div>
 ...
 <div className="mt-3 grid gap-3 md:grid-cols-[1fr,34px,1fr] md:items-center">
 <select
 value={equipeA}
 onChange={(e) => setEquipeA(e.target.value)}
 className="h-12 w-full border border-zinc-200 bg-white px-4 text-sm font-semibold outline-none"
 >
 <option value="">Selecione a equipe 1</option>
 ...
 className="h-12 w-full border border-zinc-200 bg-white px-4 text-sm font-semibold outline-none"
 >
 <option value="">Selecione a equipe 2</option>
 ...
 className="inline-flex h-12 items-center justify-center gap-2 border border-zinc-200 bg-white px-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#142340] hover:bg-[#f3f3ef]"
 >
 <RefreshCcw size={14} />
 Limpar
 </button>
 ...
 disabled={salvando}
 className="inline-flex h-12 items-center justify-center gap-2 border border-red-600 bg-red-600 px-4 text-[11px] font-black uppercase tracking-[0.16em] text-white hover:brightness-95 disabled:opacity-60"
 >
 <Swords size={14} />
 Criar confronto
 </button>
 ...
 disabled={salvando}
 className="mt-3 inline-flex h-11 w-full items-center justify-center gap-2 border border-zinc-200 bg-[#f3f3ef] px-4 text-[11px] font-semibold uppercase tracking-[0.16em] text-[#142340] hover:bg-[#dff3ff] disabled:opacity-60"
 >
 <Shuffle size={14} />
 Gerar automático
 </button>
