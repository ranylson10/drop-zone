   ...
   <div className={classNamePainel(true)}>
    <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
     <div>
      ...
      <p className="mt-2 text-sm font-semibold text-zinc-500">Gerencie os confrontos do seu campeonato.</p>
     </div>

     <div className="flex flex-col gap-2 sm:flex-row">
      <button type="button" onClick={() => setShowCriarFase((valor) => !valor)} className="inline-flex h-11 items-center justify-center gap-2 border border-zinc-200 bg-white px-4 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#142340] hover:bg-[#f3f3ef]">
       <Plus size={14} />
       Nova fase
      </button>
      <button type="button" onClick={() => setShowCriarConfronto((valor) => !valor)} className="inline-flex h-11 items-center justify-center gap-2 border border-red-600 bg-red-600 px-5 text-[11px] font-black uppercase tracking-[0.14em] text-white hover:brightness-95">
       <Plus size={14} />
       Adicionar confronto
      </button>
    ...
    {showCriarFase ? (
     <div className="mb-5 border border-zinc-200 bg-[#eceee5] p-4">
      <div className="mb-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#142340]">Nova fase</div>
      <div className="grid gap-3 md:grid-cols-[1fr_240px_auto]">
       <input value={novaFase} onChange={(e) => setNovaFase(e.target.value)} placeholder="Ex: Rodada 2, Semifinal, Final..." className="h-11 border border-zinc-200 bg-white px-3 text-sm font-semibold outline-none" />
    ...
    {showCriarConfronto ? (
     <div className="mb-5 border border-zinc-200 bg-[#eceee5] p-4">
      <div className="mb-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-[#142340]">Adicionar confronto</div>
        ...
        <div key={fase.id} className="overflow-hidden border border-zinc-200 bg-white">
         ...
         {jogosDaFase.length === 0 ? (
          <div className="p-10 text-center">
           <div className="mx-auto mb-3 flex h-11 w-11 items-center justify-center rounded-full bg-zinc-100">
            <CalendarDays size={19} className="text-zinc-500" />
           </div>
           ...
           <button type="button" onClick={() => { setFaseSelecionada(fase.id); setTipoConfronto(fase.tipo_fase || 'mata_mata'); setShowCriarConfronto(true) }} className="mt-5 inline-flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#142340]">
            <Plus size={14} />
            Adicionar confronto
           </button>
          </div>
          ...
          <div className="overflow-x-auto">
           <div className="grid min-w-[920px] grid-cols-[70px_220px_40px_220px_180px_140px_100px_70px] border-b border-zinc-200 bg-white px-4 py-3 text-[10px] font-black uppercase tracking-[0.12em] text-zinc-500">
            <div>#</div>
            <div>Equipe 1</div>
           ...
           {jogosDaFase.map((jogo: any, index: number) => {
            ...
            return (
             <div key={jogo.id} className="grid min-w-[920px] grid-cols-[70px_220px_40px_220px_180px_140px_100px_70px] items-center border-b border-zinc-200 px-4 py-4 last:border-b-0">
              <div className="text-sm font-semibold text-[#142340]">{String(jogo.ordem || jogo.numero || index + 1).padStart(2, '0')}</div>

              <select value={equipe1?.id || ''} onChange={() => {}} className="h-11 border border-zinc-200 bg-white px-3 text-sm font-semibold text-[#142340] outline-none">
               <option value={equipe1?.id || ''}>{getEquipeNome(equipe1)}</option>
               {equipes.map((equipe) => <option key={equipe.id} value={equipe.id}>{getEquipeNome(equipe)}</option>)}
              </select>

              <div className="text-center text-xl font-black text-[#142340]">X</div>

              <select value={equipe2?.id || ''} onChange={() => {}} className="h-11 border border-zinc-200 bg-white px-3 text-sm font-semibold text-[#142340] outline-none">
               <option value={equipe2?.id || ''}>{getEquipeNome(equipe2)}</option>
               {equipes.map((equipe) => <option key={equipe.id} value={equipe.id}>{getEquipeNome(equipe)}</option>)}
              </select>

              <div className="flex h-11 items-center gap-2 border border-zinc-200 bg-white px-3 text-sm font-semibold text-[#142340]">
               <CalendarDays size={15} />
               {formatarDataCurta(jogo)}
              </div>

              <div className="flex h-11 items-center gap-2 border border-zinc-200 bg-white px-3 text-sm font-semibold text-[#142340]">
               <Clock3 size={15} />
               {formatarHoraCurta(jogo)}
              </div>

              <div className="text-sm font-black uppercase text-[#142340]">MD{melhorDeTexto}<div className="text-[10px] font-semibold text-zinc-500">{qtdQuedas} queda{Number(qtdQuedas) === 1 ? '' : 's'}</div></div>

              <button type="button" onClick={() => excluirConfronto(jogo.id)} title="Excluir confronto" className="inline-flex h-10 w-10 items-center justify-center text-red-600 hover:bg-red-50">
               <Trash2 size={16} />
              </button>
             </div>
            )
           ...
           <button type="button" onClick={() => { setFaseSelecionada(fase.id); setTipoConfronto(fase.tipo_fase || 'mata_mata'); setShowCriarConfronto(true) }} className="flex h-14 min-w-[920px] items-center justify-center gap-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-zinc-500 hover:bg-zinc-50">
            <Plus size={14} />
            Adicionar confronto
           </button>
          </div>
