...
function Logo({ url, nome }: { url?: string | null; nome: string }) {
  return (
    <div className="relative h-16 w-16 shrink-0 border border-slate-200 bg-slate-100">
      {url ? (
        <Image src={url} alt={nome} fill className="object-cover" />
      ) : (
...
function dataHoraJogo(jogo: Jogo) {
  ...
  const hora =
    jogo.hora_jogo ||
    (jogo.data_inicio
      ? new Date(jogo.data_inicio).toLocaleTimeString("pt-BR", {
          hour: "2-digit",
          minute: "2-digit",
        })
      : "hora");
...
export default function EscalaVagaPage() {
  ...
  const [jogadoresCampeonato, setJogadoresCampeonato] = useState<
    JogadorCampeonato[]
  >([]);
  const [quedas, setQuedas] = useState<
    Record<string, { quedas: number; abates: number }>
  >({});
  ...
  async function carregar() {
    ...
    try {
      ...
      const { data: vagaData, error: vagaError } = await supabase
        ...
        .select(
          "id, campeonato_id, equipe_id, line_id, numero_vaga, nome_exibicao, status, campeonatos:campeonato_id(*), equipes:equipe_id(id,nome,tag,logo_url,criado_por)",
        )
      ...
      jcLista.forEach((j) => {
        if (j.perfil_jogo_id && j.status !== "removido")
          marcados[j.perfil_jogo_id] = true;
      });
      ...
      if (jcLista.length) {
        ...
        for (const row of (quedasData || []) as Quedas[]) {
          ...
          if (!partidasPorJogador[row.jogador_campeonato_id])
            partidasPorJogador[row.jogador_campeonato_id] = new Set();
          ...
          mapa[row.jogador_campeonato_id] = {
            ...
            abates:
              (mapa[row.jogador_campeonato_id]?.abates || 0) +
              Number(row.abates || 0),
          };
        }
        setQuedas(mapa);
  ...
  async function salvarEscalacao() {
    ...
    setOk(null);
    try {
      const escolhidos = Object.entries(selecionados)
        .filter(([, ativo]) => ativo)
        .map(([id]) => id);
      const existentesPorPerfil = new Map(
        jogadoresCampeonato.map((j) => [j.perfil_jogo_id, j]),
      );

      if (lineSelecionada !== (vaga.line_id || "")) {
        const { error } = await supabase
          .from("campeonato_equipes")
          .update({ line_id: lineSelecionada || null })
          .eq("id", vaga.id);
        if (error) throw error;
      }
  ...
  if (loading) {
    return (
      <main className="grid min-h-screen place-items-center bg-slate-50 px-4 text-slate-950 [color-scheme:light]">
        <div className="border border-slate-200 bg-white p-5 text-sm font-black uppercase text-slate-500">
  ...
  if (erro && !vaga) {
    return (
      <main className="min-h-screen bg-slate-50 px-4 py-8 text-slate-950 [color-scheme:light]">
        <div className="mx-auto max-w-md border border-red-200 bg-red-50 p-5 text-sm font-bold text-red-700">
          {erro}
        </div>
      </main>
    );
  ...
  const lineAtual =
    lines.find((line) => line.id === (vaga?.line_id || lineSelecionada)) ||
    null;
  ...
  return (
    <main className="min-h-screen bg-slate-50 px-3 py-4 text-slate-950 [color-scheme:light]">
      <div className="mx-auto max-w-xl pb-24">
        <Link
          href={vaga ? `/escala/${vaga.campeonato_id}` : "/"}
          className="mb-3 inline-flex items-center gap-2 text-xs font-black uppercase text-slate-500"
        >
        ...
        <section className="border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex gap-3">
            <Logo
              url={equipe?.logo_url}
              nome={vaga?.nome_exibicao || equipe?.nome || "Equipe"}
            />
            <div className="min-w-0 flex-1">
              <p className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-600">
                Escalação da vaga
              </p>
              <h1 className="truncate text-xl font-black uppercase tracking-[-0.04em]">
                {vaga?.nome_exibicao || equipe?.nome || "Vaga"}
              </h1>
              <p className="mt-1 truncate text-xs font-bold text-slate-500">
                {campeonato?.nome || "Campeonato"} • Vaga{" "}
                {vaga?.numero_vaga || "-"}
              </p>
              <p className="mt-1 text-[10px] font-black uppercase text-slate-400">
                Line atual: {lineAtual?.nome || "não definida"}
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="mt-4 border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <select
            value={lineSelecionada}
            onChange={(e) => setLineSelecionada(e.target.value)}
            className="mt-3 h-11 w-full border border-slate-200 bg-white px-3 text-sm font-bold outline-none focus:border-blue-500"
          >
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs font-semibold text-slate-500">
                Marque quem faz parte desta vaga/line no campeonato.
              </p>
            </div>
            <span className="border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] font-black uppercase text-slate-500">
              {Object.values(selecionados).filter(Boolean).length} ativos
            </span>
          </div>

          <div className="mt-3 space-y-2">
            {jogadoresAtivos.map(({ perfil, jc }) => {
              const ativo = !!selecionados[perfil.id];
              const stats = jc?.id ? quedas[jc.id] : null;
              return (
                <label
                  key={perfil.id}
                  className={`flex cursor-pointer items-center gap-3 border p-3 ${ativo ? "border-blue-300 bg-blue-50" : "border-slate-200 bg-slate-50"}`}
                >
                  <input
                    type="checkbox"
                    checked={ativo}
                    onChange={(e) =>
                      setSelecionados((old) => ({
                        ...old,
                        [perfil.id]: e.target.checked,
                      }))
                    }
                    className="h-5 w-5 accent-blue-600"
                  />
                  <div className="grid h-10 w-10 place-items-center border border-slate-200 bg-white text-slate-400">
                    <UserRound size={18} />
                  </div>
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="truncate text-[10px] font-bold uppercase text-slate-500">
                      ID: {perfil.uid_jogo || "N/I"} •{" "}
                      {perfil.funcao || "função"} •{" "}
                      {perfil.plataforma || "plataforma"}
                    </p>
                  </div>
                  <div className="text-right">
                    <b className="block text-sm font-black text-slate-950">
                      {stats?.quedas || 0}
                    </b>
            ...
            {!perfis.length ? (
              <div className="border border-dashed border-slate-300 bg-slate-50 p-4 text-center text-xs font-bold text-slate-500">
                Nenhum jogador no elenco. Adicione abaixo.
              </div>
            ) : null}
          </div>
        </section>
        ...
        <section className="mt-4 border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <form
            onSubmit={adicionarJogadorRapido}
          ...
          >
            <input
              ...
              placeholder="Nick"
              className="h-11 border border-slate-200 px-3 text-sm font-bold outline-none focus:border-blue-500"
            />
            <input
              value={novoUid}
              ...
              className="h-11 border border-slate-200 px-3 text-sm font-bold outline-none focus:border-blue-500"
            />
            <button className="flex h-11 items-center justify-center gap-2 border border-slate-950 bg-slate-950 px-4 text-xs font-black uppercase text-white">
              <Plus size={15} /> Add
            </button>
          </form>
        </section>
        ...
        <section className="mt-4 border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <p className="mt-1 text-xs font-semibold text-slate-500">
            Controle importante para regra de mínimo de quedas antes da final.
          </p>
          <div className="mt-3 space-y-2">
            {jogadoresAtivos
              .filter(({ perfil }) => selecionados[perfil.id])
              .map(({ perfil, jc }) => {
                const stats = jc?.id ? quedas[jc.id] : null;
                return (
                  <div
                    key={perfil.id}
                    className="grid grid-cols-[1fr_auto_auto] items-center gap-2 border border-slate-200 bg-slate-50 p-3"
                  >
                    <span className="truncate text-xs font-black uppercase text-slate-950">
                      {perfil.nick || "Sem nick"}
                    </span>
                    <span className="border border-slate-200 bg-white px-2 py-1 text-[10px] font-black uppercase text-slate-600">
                      {stats?.quedas || 0} quedas
                    </span>
                    <span className="border border-slate-200 bg-white px-2 py-1 text-[10px] font-black uppercase text-slate-600">
                      {stats?.abates || 0} abates
                    </span>
                  </div>
                );
              })}
          ...
          <div className="mt-3 space-y-2">
            {jogos.map((jogo) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <h3 className="truncate text-xs font-black uppercase text-slate-950">
                    {jogo.nome || `Jogo ${jogo.numero || "-"}`}
                  </h3>
        ...
        {erro ? (
          <div className="mt-4 border border-red-200 bg-red-50 p-3 text-xs font-bold text-red-700">
            <ShieldAlert size={14} className="mr-1 inline" />
            {erro}
          </div>
        ) : null}
        {ok ? (
          <div className="mt-4 border border-emerald-200 bg-emerald-50 p-3 text-xs font-bold text-emerald-700">
        ...
        <div className="fixed inset-x-0 bottom-0 border-t border-slate-200 bg-white/95 px-3 py-3 backdrop-blur [color-scheme:light]">
          <div className="mx-auto grid max-w-xl grid-cols-2 gap-2">
            {campeonato ? (
              ...
              >
                <Trophy size={14} /> Completo
              </Link>
            ) : null}
            <button
              onClick={salvarEscalacao}
              disabled={salvando}
              className="flex h-11 items-center justify-center gap-2 border border-blue-600 bg-blue-600 text-[10px] font-black uppercase text-white disabled:opacity-60"
            >
              {salvando ? (
