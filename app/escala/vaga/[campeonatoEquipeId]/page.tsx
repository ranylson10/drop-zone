...
import { ArrowLeft, CalendarDays, Check, Loader2, Plus, Save, ShieldAlert, Trophy, UserRound, Users } from 'lucide-react'
      ...
      <div className="mx-auto max-w-xl pb-24">
        ...
        <section className="mt-4 border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h2 className="text-base font-black uppercase tracking-[-0.03em]">Jogadores escalados</h2>
              <p className="mt-1 text-xs font-semibold text-slate-500">Marque quem faz parte desta vaga/line no campeonato.</p>
            </div>
            <span className="border border-slate-200 bg-slate-50 px-2 py-1 text-[10px] font-black uppercase text-slate-500">{Object.values(selecionados).filter(Boolean).length} ativos</span>
          </div>
          ...
          <div className="mt-3 space-y-2">
            {jogadoresAtivos.map(({ perfil, jc }) => {
              ...
              return (
                <label key={perfil.id} className={`flex cursor-pointer items-center gap-3 border p-3 ${ativo ? 'border-blue-300 bg-blue-50' : 'border-slate-200 bg-slate-50'}`}>
                  <input type="checkbox" checked={ativo} onChange={(e) => setSelecionados((old) => ({ ...old, [perfil.id]: e.target.checked }))} className="h-5 w-5 accent-blue-600" />
                  <div className="grid h-10 w-10 place-items-center border border-slate-200 bg-white text-slate-400"><UserRound size={18}/></div>
                  <div className="min-w-0 flex-1">
                    <h3 className="truncate text-sm font-black uppercase text-slate-950">{perfil.nick || 'Sem nick'}</h3>
                    <p className="truncate text-[10px] font-bold uppercase text-slate-500">ID: {perfil.uid_jogo || 'N/I'} • {perfil.funcao || 'função'} • {perfil.plataforma || 'plataforma'}</p>
                  </div>
                  <div className="text-right">
                    <b className="block text-sm font-black text-slate-950">{stats?.quedas || 0}</b>
                    <span className="text-[9px] font-black uppercase text-slate-400">quedas</span>
                  </div>
                </label>
              )
            })}

            {!perfis.length ? <div className="border border-dashed border-slate-300 bg-slate-50 p-4 text-center text-xs font-bold text-slate-500">Nenhum jogador no elenco. Adicione abaixo.</div> : null}
          </div>
        </section>
        ...
        <section className="mt-4 border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="text-base font-black uppercase tracking-[-0.03em]">Quedas jogadas</h2>
          <p className="mt-1 text-xs font-semibold text-slate-500">Controle importante para regra de mínimo de quedas antes da final.</p>
          <div className="mt-3 space-y-2">
            {jogadoresAtivos.filter(({ perfil }) => selecionados[perfil.id]).map(({ perfil, jc }) => {
              const stats = jc?.id ? quedas[jc.id] : null
              return (
                <div key={perfil.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-2 border border-slate-200 bg-slate-50 p-3">
                  <span className="truncate text-xs font-black uppercase text-slate-950">{perfil.nick || 'Sem nick'}</span>
                  <span className="border border-slate-200 bg-white px-2 py-1 text-[10px] font-black uppercase text-slate-600">{stats?.quedas || 0} quedas</span>
                  <span className="border border-slate-200 bg-white px-2 py-1 text-[10px] font-black uppercase text-slate-600">{stats?.abates || 0} abates</span>
                </div>
              )
            })}
          </div>
        ...
        <section className="mt-4 border border-slate-200 bg-white p-4 shadow-sm">
          <h2 className="text-base font-black uppercase tracking-[-0.03em]">Próximos jogos</h2>
          <div className="mt-3 space-y-2">
            {jogos.map((jogo) => (
              <div key={jogo.id} className="flex items-center gap-3 border border-slate-200 bg-slate-50 p-3">
                <div className="grid h-10 w-10 place-items-center border border-slate-200 bg-white text-slate-400"><CalendarDays size={16}/></div>
                <div className="min-w-0 flex-1">
                  <h3 className="truncate text-xs font-black uppercase text-slate-950">{jogo.nome || `Jogo ${jogo.numero || '-'}`}</h3>
                  <p className="text-[10px] font-black uppercase text-slate-500">{dataHoraJogo(jogo)} {jogo.rodada ? `• rodada ${jogo.rodada}` : ''}</p>
                </div>
              </div>
            ))}
            {!jogos.length ? <div className="border border-dashed border-slate-300 bg-slate-50 p-4 text-center text-xs font-bold text-slate-500">Nenhum próximo jogo cadastrado.</div> : null}
          </div>
        </section>
