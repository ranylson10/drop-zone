...
const tierPalette: Record<PlayerCardTier, { start: string; mid: string; end: string; stroke: string; badge: string }> = {
...
function getPalette(tier: PlayerCardTier, variant: PlayerCardVariant) {
  if (variant === 'avulso') {
    ...
    >
      <svg viewBox="0 0 180 250" className={`block h-full w-full ${isAvulso ? 'grayscale' : ''}`} role="img">
        ...
        <g clipPath={`url(#${id}-clip)`}>
          ...
          {photoUrl && !isSlot ? (
            <image href={photoUrl} x="34" y="34" width="112" height="118" preserveAspectRatio="xMidYMin slice" opacity={isAvulso ? .72 : 1} />
          ) : (
            <>
              <circle cx="90" cy="68" r="31" fill={isSlot ? '#1f2937' : '#020617'} />
              <path d="M25 148 C30 94 55 75 90 75 C125 75 150 94 155 148 Z" fill={isSlot ? '#1f2937' : '#020617'} />
            </>
          )}
          ...
          <path d="M12 130 H168 V160 C168 178 162 191 148 202 C127 212 110 216 99 227 L90 237 L81 227 C70 216 53 212 32 202 C18 191 12 178 12 160 Z" fill={`url(#${id}-carbon)`} opacity=".7" />
          <rect x="12" y="130" width="156" height="2" fill={p.stroke} opacity=".85" />
          <rect x="28" y="172" width="124" height="1" fill={p.stroke} opacity=".35" />
        </g>

        <path d="M90 5 C98 16 106 20 119 17 L143 12 C158 20 166 31 168 45 L168 142 C168 174 162 192 148 202 C127 212 110 216 99 227 L90 237 L81 227 C70 216 53 212 32 202 C18 192 12 174 12 142 L12 45 C14 31 22 20 37 12 L61 17 C74 20 82 16 90 5 Z" fill="none" stroke={p.stroke} strokeWidth="3" strokeLinejoin="round" />
        ...
        <text x="90" y={isSlot ? 162 : 158} textAnchor="middle" fontSize={isSlot ? 38 : 20} fontWeight="900" fill="#ffffff" style={{ paintOrder: 'stroke', stroke: 'rgba(0,0,0,.45)', strokeWidth: 2 }}>
