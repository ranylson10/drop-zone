...
function nomePerfil(perfil?: PerfilJogo | null, fallback?: string | null) {
  return perfil?.nick || fallback || "Jogador";
}
...
async function proximaOrdemLine(lineId: string) {
  ...
  const atual = Number((data || [])[0]?.ordem ?? -1);
...
export default function EscalacaoVagaPage() {
  ...
  const jogadoresAtivos = useMemo(
    () =>
      jogadores.filter(
        (j) => j.status !== "removido" && j.status !== "inativo",
      ),
    [jogadores],
  );
  ...
  const idsPendentes = useMemo(
    () =>
      new Set(
        convites.map((c) => c.perfil_jogo_id).filter(Boolean) as string[],
      ),
    [convites],
  );
  const slots = useMemo(
    () =>
      Array.from({ length: limiteJogadores }).map(
        (_, i) => jogadoresAtivos[i] || null,
      ),
    [jogadoresAtivos, limiteJogadores],
  );
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const { data: vagaData, error: vagaError } = await supabase
        ...
        .select(
          "id,campeonato_id,equipe_id,line_id,numero_vaga,nome_exibicao,status,grupo_id",
        )
      ...
      const [{ data: campData }, { data: equipeData }] = await Promise.all([
        supabase
          .from("campeonatos")
          .select(
            "id,nome,slug,logo_url,status,jogadores_por_equipe,reservas_permitidos,plataforma,tipo,tipo_campeonato",
          )
      ...
      if (uid && equipeAtual?.id) {
        if (equipeAtual.criado_por === uid) {
          ...
          permissao = ((membro || []) as any[]).some((item) => {
            const perfil = Array.isArray(item.perfis_jogo)
              ? item.perfis_jogo[0]
              : item.perfis_jogo;
            return (
              perfil?.user_id === uid &&
      ...
      setJogadores(
        listaJogadores.map((j) => ({
          ...j,
          perfil: j.perfil_jogo_id
            ? mapaPerfis.get(j.perfil_jogo_id) || null
            : null,
        })),
      );
      ...
      if (equipeAtual?.id) {
        const [
          ...
          supabase
            ...
            .select(
              "id,perfil_jogo_id,status,tipo,created_at,perfil:perfil_jogo_id(*)",
            )
            .eq("equipe_id", equipeAtual.id)
            .eq("status", "pendente")
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      ...
      setCandidatos(
        ((data || []) as PerfilJogo[]).filter((p) => !bloqueados.has(p.id)),
      );
    }, 300);
  ...
  async function escalarPerfil(perfil: PerfilJogo) {
    ...
    if (!canManage)
      return alert("Você não tem permissão para escalar esta equipe.");
    if (jogadoresAtivos.length >= limiteJogadores)
      return alert("Todos os slots da escalação já estão preenchidos.");

    try {
      setProcessando(perfil.id);
      await validarAlteracaoEscala(campeonato.id, "adicionar");
      ...
      const { data: jogadorId, error: garantirError } = await supabase.rpc(
        "fn_garantir_jogador_campeonato_vaga",
        {
          ...
          p_campeonato_equipe_id: vaga.id,
      ...
      const { error: ativarError } = await supabase
        ...
        .update({
          ...
          observacoes:
            ...
            Number(campeonato.jogadores_por_equipe || 4)
              ? "titular"
              : "reserva",
        })
        .eq("id", jogadorId);
      ...
      if (vaga.line_id) {
        ...
        if (!existenteLine?.id) {
          const ordem = await proximaOrdemLine(vaga.line_id);
      ...
      await carregar();
      alert("Jogador escalado na line/vaga do campeonato.");
    } catch (error: any) {
  ...
  async function removerJogador(jogador: JogadorCampeonato) {
    if (!vaga || !campeonato) return;
    if (!canManage)
      return alert(
        "Você não tem permissão para remover jogador desta escalação.",
      );
    const ok = confirm(
      `Remover ${nomePerfil(jogador.perfil, jogador.nick_snapshot)} da escalação?`,
    );
    if (!ok) return;
    ...
    try {
      ...
      alert(error?.message || error?.details || "Erro ao remover jogador.");
    } finally {
      setProcessando(null);
    }
  }
  ...
  async function enviarConvite(perfil: PerfilJogo) {
    ...
    if (!canManage)
      return alert(
        "Você não tem permissão para convidar jogadores para esta equipe.",
      );
    ...
    try {
      ...
      await carregar();
      alert(
        "Convite enviado. Quando o jogador aceitar, ele poderá ser escalado aqui.",
      );
    } catch (error: any) {
      ...
      <div className="mx-auto max-w-md pb-10">
        <section className="overflow-hidden border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-blue-500 bg-gradient-to-r from-blue-600 to-blue-700 p-4 text-white">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 truncate text-xs font-bold text-blue-100">
                  {campeonato.nome}
                </p>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 border-b border-slate-200">
            <Info
              label="Vaga"
              value={vaga.numero_vaga ? `#${vaga.numero_vaga}` : "Inscrita"}
            />
            <Info
              label="Slots"
              value={`${jogadoresAtivos.length}/${limiteJogadores}`}
            />
        ...
        {!userId ? (
          <section className="mt-3 border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">
              Para escalar, convidar jogador ou aceitar convite, use o
              login/cadastro original do site.
            </p>
            <div className="mt-4 grid grid-cols-2 gap-2">
              ...
              <Link
                href="/cadastro"
                className="flex h-11 items-center justify-center border border-slate-300 bg-white text-xs font-black uppercase text-slate-800"
              >
                Criar conta
              </Link>
            </div>
          ...
          <section className="mt-3 border border-yellow-300 bg-yellow-50 p-4 text-xs font-bold leading-5 text-yellow-900">
            Esta tela é da equipe, mas sua conta não tem permissão de
            dono/admin/manager para editar a escalação. Você ainda pode abrir
            seus convites abaixo.
          </section>
        ) : null}
          ...
          <div className="flex items-center justify-between border-b border-white/10 bg-[#0b1628] px-3 py-3">
            <div>
              ...
              <p className="text-xs font-black uppercase text-white">
                {jogadoresAtivos.length}/{limiteJogadores} cards usados
              </p>
            </div>
            <button
              onClick={copiarLink}
              className="flex h-9 items-center justify-center gap-1 border border-white/10 bg-white/10 px-3 text-[10px] font-black uppercase text-white"
            >
              <Copy size={14} /> Link
            </button>
          </div>
            ...
            {slots.map((jogador, index) => (
              <div key={jogador?.id || `slot-${index}`} className="relative">
                <PlayerCard
                  name={
                    jogador
                      ? nomePerfil(jogador.perfil, jogador.nick_snapshot)
                      : ""
                  }
        ...
        <section className="mt-3 border border-slate-200 bg-white p-3 shadow-sm">
          <div className="mb-3 flex items-center justify-between gap-2">
            <div>
              ...
              <h2 className="text-base font-black uppercase tracking-[-0.03em]">
                Escalar jogadores da equipe
              </h2>
            </div>
            ...
            <div className="space-y-2">
              {elenco.map((perfil) => {
                ...
                return (
                  <div
                    key={perfil.id}
                    className="flex items-center gap-3 border border-slate-200 bg-slate-50 p-2"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] font-bold text-slate-500">
                        UID {uidPerfil(perfil)} •{" "}
                        {perfil.funcao || "Sem função"}
                      </p>
                    </div>
                    {escalado ? (
                      <span className="inline-flex h-9 items-center gap-1 border border-emerald-200 bg-emerald-50 px-2 text-[10px] font-black uppercase text-emerald-700">
                        <CheckCircle2 size={13} /> Escalado
                      </span>
                    ) : (
                      <button
                        onClick={() => escalarPerfil(perfil)}
                        disabled={
                          !canManage ||
                          processando === perfil.id ||
                          jogadoresAtivos.length >= limiteJogadores
                        }
                        className="inline-flex h-9 items-center gap-1 border border-blue-600 bg-blue-600 px-3 text-[10px] font-black uppercase text-white disabled:opacity-50"
                      >
                        {processando === perfil.id ? (
        ...
        <section className="mt-3 border border-slate-200 bg-white p-3 shadow-sm">
          ...
          <textarea
            ...
            className="mt-2 min-h-20 w-full border border-slate-200 bg-white p-3 text-sm font-semibold outline-none placeholder:text-slate-400"
          />
            ...
            {candidatos.map((perfil) => (
              ...
              >
                ...
                <button
                  ...
                  disabled={
                    !canManage || processando === `convite-${perfil.id}`
                  }
                  className="inline-flex h-9 items-center gap-1 border border-violet-600 bg-violet-600 px-3 text-[10px] font-black uppercase text-white disabled:opacity-50"
                >
                  {processando === `convite-${perfil.id}` ? (
                    <Loader2 size={13} className="animate-spin" />
                  ) : (
                    <MailPlus size={13} />
                  )}{" "}
            ...
            <div className="mt-4 border-t border-slate-100 pt-3">
              ...
              <div className="space-y-2">
                {convites.map((convite) => (
                  <div
                    key={convite.id}
                    className="flex items-center gap-2 border border-yellow-200 bg-yellow-50 p-2 text-xs font-bold text-yellow-900"
                  >
                    <XCircle size={14} />
        ...
        <section className="mt-3 grid grid-cols-2 gap-2">
          <Link
            href={`/escala/${campeonato.slug || campeonato.id}`}
          ...
          <Link
            ...
            className="flex h-11 items-center justify-center gap-2 border border-blue-600 bg-blue-600 text-[10px] font-black uppercase text-white"
          >
            <UserPlus2 size={14} /> Convites
          </Link>
        </section>
  ...
  return (
    <div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden border border-slate-200 bg-white text-xs font-black text-slate-500">
      {perfil.foto_capa ? (
        ...
        nomePerfil(perfil).slice(0, 2).toUpperCase()
      )}
    </div>
  );
}
...
function CardAviso({ texto }: { texto: string }) {
  return (
    <div className="border border-slate-200 bg-slate-50 p-3 text-xs font-semibold leading-5 text-slate-500">
      {texto}
    </div>
  );
}
