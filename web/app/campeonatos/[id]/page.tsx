...
export default function CampeonatoDetalhePage({ tipoForcado }: { tipoForcado?: string } = {}) {
  ...
  const [pixCompraPendente, setPixCompraPendente] = useState(false)
  ...
  async function comprarVagaComPix() {
    ...
    try {
      ...
      if (error) {
        logErroSupabase('Erro ao reservar vaga para PIX', error)
        throw error
      }
      ...
      if (referenciaId) {
        const { error: updateInscricaoError } = await supabase
          ...
          .update({
            ...
            status_pagamento: valorVagaCompra > 0 ? 'pendente' : 'gratuito',
      ...
      setMensagemCompraVaga(
        valorVagaCompra > 0
          ? 'Vaga reservada. Gere o PIX para confirmar o pagamento.'
              ...
              <div className="space-y-4 p-5">
                ...
                {!equipeSelecionadaCompra ? (
                  ...
                  <>
                    ...
                    <div>
                      <label className="mb-1 block text-[10px] font-black uppercase tracking-widest text-zinc-500">Pagamento por PIX</label>
                      <input type="text" value="Pagamento PIX direto" readOnly className="w-full border border-slate-200 bg-white px-4 py-3 text-sm font-bold outline-none focus:bg-lime-50" placeholder="PIX será gerado na próxima etapa" readOnly />
                      <p className="mt-2 text-[11px] font-semibold text-zinc-500">A vaga fica pendente até confirmação do pagamento PIX.</p>
                    </div>
                  </>
                )}
        ...
        {comprovanteCompra && (
          <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/75 px-4">
            <div className="w-full max-w-lg dz-panel bg-white text-slate-950">
              <div className="border-b-2 border-black bg-[#7cfc00] p-5 text-center"><CheckCircle2 className="mx-auto mb-2" size={34} /><div className="text-[10px] font-black uppercase tracking-[0.25em]">Reserva criada</div><div className="text-2xl font-black uppercase italic">Reserva da vaga</div></div>
          ...
          <div className="border-b border-zinc-200 bg-white px-4 py-3 md:px-6">
            <div className={`grid gap-3 lg:items-end ${statusPermiteCompraVaga ? 'lg:grid-cols-[minmax(0,1fr)_300px_170px]' : 'lg:grid-cols-[minmax(0,1fr)_300px]'}`}>
              <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
                ...
                <LealtMiniInfo label="PIX" value={formatarMoeda(pixPIX)} />
