...
function BadgeStatus({ status }: { status?: string | null }) {
  ...
  const estilos: Record<string, string> = {
    ...
    em_andamento: 'border-sky-200 bg-sky-50 text-[#142340]',
...
export default function CampeonatoDiarioDetalhePage({ tipoForcado = 'diario' }: { tipoForcado?: string } = {}) {
  ...
  const [pixPendenteInfo, setPixPendenteInfo] = useState<string | null>(null)
  ...
  async function comprarVagaComPix() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(json?.error || 'Erro ao reservar vaga para pagamento PIX.')
      ...
      console.error('Erro ao reservar vaga com PIX:', e)
      setErro(e?.message || e?.details || 'Erro ao reservar vaga para pagamento PIX.')
    } finally {
      setSalvandoSlot(false)
    }
  }
  ...
  const visual = getTipoVisual(tipoForcado)
  const tipoVisualKey = visual.key
  const headerGradient =
    tipoVisualKey === 'xtreino'
      ? 'border-emerald-900/20 bg-gradient-to-r from-emerald-700 via-emerald-600 to-emerald-900'
      : tipoVisualKey === 'copa'
        ? 'border-violet-900/20 bg-gradient-to-r from-violet-700 via-violet-600 to-violet-900'
        : tipoVisualKey === 'liga'
          ? 'border-amber-900/20 bg-gradient-to-r from-amber-600 via-amber-500 to-amber-800'
          : false
            ? 'border-red-900/20 bg-gradient-to-r from-red-700 via-red-600 to-red-900'
            : 'border-sky-900/20 bg-gradient-to-r from-sky-700 via-blue-700 to-sky-900'
  const tituloTipo = tipoVisualKey === 'xtreino' ? 'Xtreino jogo único' : 'Diário por horários'
  ...
  return (
    <div className="mx-auto max-w-7xl px-3 py-4 text-[#142340] max-md:px-0 max-md:py-0">
      ...
      <div className={`mb-4 overflow-hidden border ${headerGradient} text-white shadow-sm`}>
          ...
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              <div className="mb-2 flex flex-wrap items-center gap-2">
                ...
                <span className={`inline-flex items-center border border-white/25 bg-white/90 px-2 py-1 text-[9px] font-semibold uppercase tracking-[0.18em] ${visual.text}`}>
                  Mobile
                </span>
              </div>
                  ...
                  <div className={`grid ${podeGerenciar ? 'grid-cols-4' : 'grid-cols-3'} border-b border-zinc-200 bg-zinc-50 p-2`}>
                    <button
                      ...
                      className={`h-11 border text-[11px] font-semibold uppercase tracking-[0.14em] ${
                        abaDireita === 'equipes'
                          ? `${visual.accent} border-transparent text-white`
                          : `border-zinc-200 bg-white text-zinc-600 hover:bg-zinc-50 ${visual.text}`
                      }`}
                    >
                      Equipes
                    </button>
                        ...
                        <div className="mb-3 grid gap-2 md:grid-cols-[minmax(0,1fr)_170px]">
                          {podeGerenciar ? (
                            <button
                              ...
                              className={`inline-flex h-11 items-center justify-center gap-2 border px-3 text-[12px] font-semibold uppercase tracking-[0.08em] transition ${visual.buttonOutline}`}
                            >
                              <UserPlus size={14} />
                            ...
                            <button
                              ...
                              className={`inline-flex h-11 items-center justify-center gap-2 border px-3 text-[12px] font-semibold uppercase tracking-[0.08em] transition ${visual.button} disabled:cursor-not-allowed disabled:border-zinc-200 disabled:bg-zinc-100 disabled:text-zinc-400`}
                            >
                              <QrCode size={14} />
                          ...
                          {!podeGerenciar ? (
                            <div className={`flex h-11 items-center border px-3 text-[12px] font-semibold ${visual.chip}`}>
                              PIX direto
                            </div>
                          ) : (
                              ...
                              return (
                                <div
                                  ...
                                  className={`grid grid-cols-[38px_38px_minmax(0,1fr)_auto] items-center gap-2 border px-2 py-2 text-left transition ${
                                    ocupado
                                      ? `${visual.border} ${visual.bg} hover:brightness-[0.98]`
                                ...
                                >
                                  <div className={`flex h-9 w-9 items-center justify-center border text-[12px] font-semibold ${
                                    ocupado
                                      ? `${visual.border} bg-white ${visual.text}`
                                  ...
                                  {podeComprarSlot ? (
                                    <button
                                      ...
                                      className={`h-9 border px-3 text-[10px] font-semibold uppercase tracking-[0.08em] transition ${visual.button}`}
                                    >
                                      Comprar
                                    </button>
                                  ) : (
                          ...
                          {linhasMvp.length > 0 ? (
                            linhasMvp.slice(0, 12).map((item, index) => {
                              ...
                              return (
                                <div
                                  key={item.key}
                                  className={`grid grid-cols-[48px_minmax(0,1fr)_72px_72px] items-center border-b border-zinc-100 px-3 py-2 text-[12px] last:border-b-0 ${destaque ? visual.bgSoft : 'bg-white'}`}
                                >
                                  <div>
                            ...
                            quedasDoGrupo.map((queda) => (
                              <button
                                ...
                                className={`px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.12em] ${
                                  quedaAtiva === queda.id ? `${visual.accent} text-white` : 'border border-zinc-200 bg-white text-zinc-600'
                                }`}
                              >
                                {queda.titulo}
                              </button>
              ...
              {abaLateral === 'estatisticas' ? (
                <div className="flex-1 overflow-y-auto p-3">
                  ...
                  <select
                    ...
                    className="mb-3 h-10 w-full border border-zinc-200 bg-white px-3 text-[12px] font-semibold text-[#142340] outline-none focus:border-zinc-500"
                  >
                    {quedasDoGrupo.length > 0 ? (
                      quedasDoGrupo.map((queda) => (
                  ...
                  <div className="mb-3 grid grid-cols-3 gap-2">
                    <div className={`border p-2 ${visual.border} ${visual.bg}`}>
                      <div className="text-[9px] font-bold uppercase tracking-[0.12em] text-[#142340]">Equipes</div>
                          ...
                          return (
                            <div key={`stats-equipe-${linha.nome}-${index}`} className="border border-zinc-100 bg-zinc-50 p-2">
                              <div className="mb-2 flex items-center justify-between gap-2">
                                <div className="flex min-w-0 items-center gap-2">
                                  <span className={`flex h-6 w-6 items-center justify-center ${visual.accent} text-[11px] font-bold text-white`}>{linha.posicao || index + 1}</span>
                                  <span className="truncate text-[11px] font-bold uppercase text-[#142340]">{linha.nome}</span>
                                </div>
                                <span className="text-[11px] font-bold text-[#142340]">{linha.pontos} pts</span>
                              </div>
                              <div className="mb-1 h-2 bg-white">
                                <div className={`h-2 ${visual.accent}`} style={{ width: `${larguraPontos}%` }} />
                              </div>
                  ...
                  <div className="border-t border-zinc-200 p-3">
                    <div className="flex gap-2">
                      <input
                        ...
                        className="h-10 min-w-0 flex-1 border border-zinc-200 bg-white px-3 text-[12px] text-[#142340] outline-none focus:border-zinc-500"
                      />
                      <button
                        onClick={enviarMensagemChat}
                        className={`flex h-10 w-10 items-center justify-center border text-white hover:brightness-110 ${visual.button}`}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4">
          <div className="w-full max-w-lg border border-zinc-200 bg-white p-4 ">
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                ...
                <h3 className="mt-1 text-lg font-semibold text-[#142340]">
                  {modoSlotModal === 'comprar' ? 'Reservar vaga via PIX' : slotModal.equipe ? 'Editar slot' : 'Adicionar participante'}
                </h3>
              </div>
            ...
            <div className="mb-4">
              ...
              {tipoParticipanteSlot === 'equipe' ? (
                <select
                  ...
                  className="h-10 w-full border border-zinc-200 bg-white px-3 text-[12px] font-medium text-[#142340] outline-none focus:border-zinc-500"
                >
            ...
            {modoSlotModal === 'comprar' ? (
              <div className="space-y-2">
                <div className={`border px-3 py-2 text-[12px] ${visual.chip}`}>
                  Pagamento via PIX: <strong>{moeda(resumoGrupo?.inscricao || 0)}</strong>
                  {pixPendenteInfo ? ` • ${pixPendenteInfo}` : ''}
                </div>
                <button
                  onClick={comprarVagaComPix}
                  disabled={!(tipoParticipanteSlot === 'line' ? lineSelecionadaId : equipeSelecionadaId) || salvandoSlot}
                  className={`inline-flex h-10 w-full items-center justify-center gap-2 border text-[12px] font-semibold uppercase tracking-[0.08em] transition ${visual.button} disabled:cursor-not-allowed disabled:opacity-50`}
                >
                  <QrCode size={14} />
                  {salvandoSlot ? 'Processando...' : 'Gerar reserva PIX'}
                </button>
              </div>
            ) : (
              <div className="grid gap-2 md:grid-cols-2">
                <button
                  ...
                  disabled={!(tipoParticipanteSlot === 'line' ? lineSelecionadaId : equipeSelecionadaId) || salvandoSlot}
                  className={`inline-flex h-10 items-center justify-center gap-2 border text-[12px] font-medium transition ${visual.button} disabled:cursor-not-allowed disabled:opacity-50`}
                >
                  <Plus size={14} />
