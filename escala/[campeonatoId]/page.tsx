...
export default function EscalaCampeonatoPage() {
  ...
  const [aba, setAba] = useState<"menu" | "escala" | "equipe" | "jogador">("menu");
  ...
  return (
    <main className="lealt-escala-mobile min-h-screen bg-slate-50 px-2 py-2 text-slate-950 [color-scheme:light]">
      <div className="mx-auto max-w-md pb-5">
        <section className="overflow-hidden border border-slate-200 bg-white shadow-sm">
          ...
          <div className="lealt-escala-info-grid grid grid-cols-4 border-b border-slate-200 bg-white">
            <Info
              label="Status"
              value={abertas ? "Abertas" : "Fechadas"}
            />
        ...
        {!userId ? (
          ...
          <>
            {aba === "menu" ? (
              <section className="lealt-escala-actions-grid mt-2 grid grid-cols-5 gap-1">
                <button
                  onClick={() => setAba("escala")}
                  className="flex h-11 flex-col items-center justify-center gap-0.5 border border-blue-600 bg-blue-600 px-1 text-[8px] font-black uppercase leading-none text-white"
                >
                  <ListChecks size={13} />
                  Escalar
                </button>
                <button
                  ...
                  className="flex h-11 flex-col items-center justify-center gap-0.5 border border-slate-200 bg-white px-1 text-[8px] font-black uppercase leading-none text-slate-800"
                >
                  <Users size={13} />
                  Equipe
                </button>
              ...
              <button
                onClick={() => setAba("menu")}
                className="mt-2 flex h-9 w-full items-center justify-center gap-2 border border-slate-200 bg-white text-[10px] font-black uppercase text-slate-700"
              >
            ...
            {aba === "escala" ? (
              <section className="mt-2 space-y-2">
                ...
                {equipes.map((equipe) => {
                  ...
                  return (
                    <div key={equipe.id} className="border border-slate-200 bg-white p-2 shadow-sm">
                ...
                {equipes.length && !equipes.some((equipe) =>
                      ...
                      <div className="flex items-center gap-2 p-2">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-0.5 truncate text-[9px] font-black uppercase text-slate-500">
                            {inscrita ? "Inscrita neste campeonato" : "Sem vaga neste campeonato"}
                          </p>
                        </div>
                        {inscrita ? <CheckCircle2 className="text-emerald-500" size={18} /> : null}
                      </div>
                ...
                {!perfilJogo ? (
                  ...
                  <div className="border border-slate-200 bg-white p-3 shadow-sm">
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-1.5">
                      ...
                      <InfoBox label="MVP" value={posicaoMvp ? `#${posicaoMvp}` : "-"} />
                    </div>

                    <Link
                      href={perfilJogo?.id ? `/jogadores/${perfilJogo.id}` : "/jogadores"}
                      className="mt-2 flex h-10 items-center justify-center gap-1 border border-blue-600 bg-blue-600 text-[10px] font-black uppercase text-white"
                    >
                      <Gamepad2 size={14} /> Abrir perfil
                    </Link>
                  </div>
                )}
              </section>
...
function EscalacaoCards({
  ...
  const hrefAdicionar = vagaPrincipal
    ...
    : getHrefCampeonatoSeguro(campeonato);

  const [menuAberto, setMenuAberto] = useState<string | null>(null);
  const [infoAberta, setInfoAberta] = useState<JogadorEquipe | null>(null);
  const [removendoId, setRemovendoId] = useState<string | null>(null);

  async function removerJogador(jogador: JogadorEquipe) {
    if (!jogador?.id || removendoId) return;
          ...
          return (
            <div key={slotId} className="relative min-w-0">
              <PlayerCard
                ...
                onClick={() => {
                  if (!jogador) {
                    window.location.href = hrefAdicionar;
                    return;
                  }

                  setInfoAberta(null);
                  setMenuAberto(menuAberto === jogador.id ? null : jogador.id);
                }}
              />
      ...
      {infoAberta ? (
        <div className="border-t border-slate-200 bg-slate-50 p-2">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              <h4 className="truncate text-sm font-black uppercase text-slate-950">
                {getNomePerfil(infoAberta.perfil, infoAberta.nick_snapshot)}
              </h4>
              <p className="text-[10px] font-bold text-slate-500">
                UID {getUidPerfil(infoAberta.perfil, infoAberta.uid_jogo_snapshot)}
              </p>
            </div>
            <button
              type="button"
              onClick={() => setInfoAberta(null)}
          ...
          <div className="mt-2 grid grid-cols-3 gap-1">
            <InfoMini label="Tipo" value={infoAberta.jogador_avulso_id ? "Avulso" : "Oficial"} />
          ...
          {infoAberta.perfil_jogo_id ? (
            <Link
              ...
              className="mt-2 flex h-8 items-center justify-center border border-blue-600 bg-blue-600 text-[9px] font-black uppercase text-white"
            >
